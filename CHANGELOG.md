...
**Current version:** v3.80 Beta.

---

## v3.80
The release that rolls up patches 181–195 of v3.76 Beta. The whole record of
each patch stays below; the headlines:
- **The Proxy terminal reads the record.** Every Dialogue or Combat record
  writes its own sentences under itself the moment it is kept; a voiced line
  replaces its written twin with the audio id and the realtime figure; a
  sentence never voiced keeps its row. Placement is by sentence, references
  name the reserved record, and the boxes are right (p181–p190, p193).
- **A restart is not amnesia.** The previous session's spoken lines, the
  voice ledger and the takes all survive a restart; the terminal replays the
  take kept in the Saved Audio Folder, which now keeps 250 per character
  (p189, p194).
- **The panel checks its own work.** A new `audit` source under Log › Errors
  files a finding where a fault would otherwise show as a silent empty; one
  fault is logged once, then with a count; Log › Diagnostics reads the
  session back with a copyable report (p191, p194).
- **The self-test.** Run self-test exercises the folder watcher, the busy
  probe, a copy in progress, settings under contention, log rotation, the
  DLL swap, the audio folder, accented names, PowerShell and the GPU query in
  a scratch folder, in its own thread (p192, p193).
- **No updater while its own server is up** - the rule the self-test proved
  necessary (p195). **No request can freeze the page** (p193).
- Action rows in SkyrimNet's current form, `Speaker > Listener` rows from the
  target selector, inked brackets, Targets in the Options menu (p186, p187).

- **No updater while its own server is up.** The llama.cpp updater will not
  begin while any llama.cpp server is starting, running or stopping - a held
  DLL can be neither replaced nor renamed away, as the self-test on a real
  machine proved - and the audio.cpp updater will not begin while the TTS
  server is. Each looks only at its own server: the llama.cpp updater never
  checks the TTS server and the audio.cpp updater never checks the llama.cpp
  slots. The Update button is disabled and a red line names the server and
  what to do (Terminate on the top bar; Stop on the TTS page); the endpoint
  refuses as well, so nothing can begin an update round the button. The
  panel updater needs no such rule: it swaps PandorumLLM.exe alone, and both
  servers outlive a panel restart (the TTS server is adopted back).

## v3.76 patch194
- **One audio system.** Patch189 had made a second folder, `logs\tts-audio`,
  for the Proxy terminal to replay lines from, beside the Saved Audio Folder
  that already keeps every take with a readable name. That folder is gone: the
  terminal now replays the take kept in the Saved Audio Folder (found by the
  line's audio id, remembered in `logs\tts-takes.json` across restarts), the
  wrapper's served file is back in a temp folder, and **Kept Takes per
  Character defaults to 250** - the tooltip says why: the terminal replays
  from these files, and a long scene voices a few hundred lines per character.
  Delete `logs\tts-audio` yourself; the panel does not remove folders it did
  not just make.
- **A terminal that loses its records now says which leg failed.** The Proxy
  terminal once showed only spoken lines, as lone bundles, with no records
  and no way to tell why; the server had written the records and returns them
  on the same files, so the loss happened between the request and the paint
  and nothing recorded it. Three instruments: a failed tail request is filed
  under Log > Errors as `audit` with the error and painted as a failure (it
  was painted as quiet text above the scrolled view); a dashboard answer with
  no record row beside a TTS tail full of spoken lines is filed with what was
  received; and the panel audits its own dashboard tail when a file of
  kilobytes yields no record row. When the Providers filter is what hides
  every record, the terminal says that on its first line and Diagnose lists
  the hidden providers.
- "Longest kept sample, in seconds" has a tooltip and its "?".

## v3.76 patch193
- **Fixed: the whole page froze after pressing Run self-test (patch192)** -
  launcher buttons, the allocator, the terminals, the statistics and the
  launcher creator all stopped updating until the browser was refreshed. One
  request that never answered was holding the page: every redraw waits for
  the requests the page has out, and the self-test's watcher probe had hung
  on Windows. Two fixes, both permanent: no request from the page can hold it
  any more (a call that has not answered in 90 seconds is abandoned and
  reported as an error), and the self-test now runs in a thread of its own
  while the page polls for its report. The watcher probe ends its watch
  cooperatively instead of closing a handle from outside.
- **Fixed: the Proxy terminal's boxes since patch190** - spoken lines under
  the wrong record, an ActionEval row under another provider, a player's line
  swallowing a reply's rows, and rows doubled with a "Request" chip. The
  sentences a record writes under itself no longer carry a reference (that is
  what painted the chip); a voiced line replaces its written twin even when
  SkyrimNet split the sentence differently; each record's replacement and
  insertion happen in one bottom-up pass so no index moves under another;
  and a burst with no twin goes past the record's written rows instead of
  between them.

## v3.76 patch192
- **The self-test.** *Run self-test* on Log > Diagnostics - or
  `PandorumLLM.exe --selftest` from a console - exercises the panel's own
  mechanisms on this machine in about ten seconds: the folder watcher, the
  busy-file probe, a copy in progress, settings saved under contention, log
  rotation with a file held, the DLL swap strategies, the audio folder, an
  accented file name, PowerShell, the GPU clock query. Every probe works in a
  scratch folder under the logs that is removed afterwards; it reads real
  things only where reading changes nothing. The report is saved as
  `logs\selftest-<stamp>.txt` and shown for copying. A failure that only a
  panel fault explains - the watcher not waking, the busy probe answering
  wrong - is also filed under Log > Errors as `audit`. It refuses to run
  while a model scan or an update is under way.

## v3.76 patch191
- **The panel checks its own work.** A new error source, `audit`, files a
  finding where a fault would otherwise show as a silent empty: a spoken line
  whose reference names no kept record; a record row written without its id;
  an action reply, a target-selector answer or a reply's sentences that yield
  no row; a terminal with speech records on screen and no spoken lines to
  hang under them. Each carries what happened and what to do, like every
  other entry under Log > Errors.
- **One fault, once.** The same message is written once, then with its count
  at 2, 10, 100 and 1000 - not fourteen identical lines.
- **Log > Diagnostics** (the former Debug Report tab): "Diagnose this session"
  reads the session's logs back - records against references, rows against
  ids, what the replies said against what TTS voiced, the speech numbers, the
  errors by source, the audit findings - with a Copy report button for bug
  reports. The page trace recorder stays beneath it.
- The thought voice says which check failed ("no sample is recorded for X in
  the ledger") instead of guessing at a cause; the previous session's log is
  found by the stamp in its name, not by modification time.

## v3.76 patch190
- **Fixed: no spoken lines at all in the Proxy terminal (patch189).** The
  previous session ended in the evening and the new one began in the morning;
  the terminal's deeper TTS read compared clock times across the two files
  and a midnight, and dropped every line of the new day. No clock is compared
  any more.
- **The record says what was said.** Every kept Dialogue or Combat record now
  writes its own spoken sentences under itself the moment it is kept - tags
  read as words, each stage direction its own row - so the lines are in the
  box whether or not TTS ever voices them. When TTS voices a sentence, the
  voiced line takes the written row's place, with its audio and its (⚡ Nx)
  figure; a sentence never voiced keeps its row, plain, with no figure.

## v3.76 patch189
- **A restart is not amnesia.** After a restart the Proxy terminal showed the
  previous session's records without their spoken lines, and the thought
  voice said "no voice learned" for characters whose sample was on disk.
  Three fixes: the name-to-sample ledger is read from disk before the thought
  voice answers; the spoken lines (with their realtime figures) are read from
  the previous session's TTS log as well as this one's, as the dashboard
  already was; and the generated line audio now lives beside the panel's logs
  (`tts-audio`) instead of TEMP and keeps 250 lines by default (setting
  `ttsAudioKeep`, floor 24), so a line from an earlier session can still be
  played.
- The Thinking chip on a record row is centred at every terminal scale.

## v3.76 patch188
- **Fixed: the (⚡ 3.07x) realtime figure was missing from every spoken and
  thought line in the Proxy terminal** since patch186. The deeper TTS read
  that patch added kept only the spoken lines, and the figure is read from
  the line that follows each one. Those lines are kept again; the rows look
  and work as they did.

## v3.76 patch187
- **Fixed: duplicated action rows.** Patch186 taught the terminal SkyrimNet's
  ACTION form and every reply carrying one grew a second row - Combat and
  Dialogue beside their embedded action, GM and AE-Resolve where none was
  asked for. The ⚡ row is the action evaluator's alone again.
- **The target selector's row is always `Speaker > Listener`**, whatever shape
  the model answered in - backticks, brackets, an unbalanced bracket,
  "player" or "Player" - a name keeps its own brackets, the player wears the
  player's name once it is known, `0` reads "no one speaks next", and the row
  carries no emoji. The "no one speaks nex > o one speaks next" split is gone.
- **Targets** has a switch in the Options menu, beside Actions.
- **The answers inside an action's brackets are inked**: each value in the
  highlight (`upstairs`, `true`), a number in the number ink, each key in the
  tag ink, and every bracket, colon and comma white.

## v3.76 patch186
- **Action rows are back.** SkyrimNet's action providers answer in the
  `ACTION: Name PARAMS: {...}` form now; the panel read the old strict JSON and
  showed nothing. ActionEval, SeverActions and AE-Resolve (its verdict list)
  each make a row under their call again, and the Actions switch on the
  terminal bar does what it says.
- **New: Targets.** The Meta call that decides who speaks next - known by its
  prompt, since every Meta call shares the title - gets a row under it:
  `🎯 Serana > Maxxor`, or "no one speaks next". A Targets switch beside
  Actions shows or hides them.
- The terminal keeps **250** payloads, up from 150.
- **Folder Settings**: each field is one block with the same rule the server
  cards draw between their groups, equal air above and below; a title sits on
  its own field; the llama.cpp row has Open folder too.
- **Fixed: "launch did not finish" errors at every start-up.** They came from
  the previous session's server logs, quiet by definition. Only a log written
  this session can raise one.
- **Payload viewer**: a reply is shown as three paragraphs - the spoken part,
  the thought, the action - with a blank line between.
- **Fixed: spoken lines missing from a box whose reply had them.** The Proxy
  terminal spliced spoken lines from the last 400 lines of the TTS log, which
  writes five lines around every spoken one - so a record still on screen had
  its lines fall off the end, and its box kept only the thought. The splice
  now asks for every spoken line back to the oldest record on screen.

## v3.76 patch185
- **Fixed: patch184 dropped the record id from every Proxy terminal row.** A
  block landed under the wrong branch, so no record row carried its id: a
  click on a title fell through to the provider link (Live Network) instead of
  opening the payload, and spoken lines - whose references were right at last
  - had no row to hang on. One build; fixed with a check that reads the row
  back from the log file.
- Folder Settings: one "Sweep launcher folder" button, its explanation on
  hover; the title above it and the "Sweep now" button were a duplicate.

## v3.76 patch184
- **Fixed: spoken lines outside their reply's box, and two replies' lines
  merged into one.** The cause was the panel's own, since patch179: every
  spoken line named a record id one short of the record it belonged to, so
  its reference pointed at nothing and the terminal fell back to the clock -
  and, with a reference on every line, it read the reference as the speaker,
  so lines of different characters spoken within seconds were treated as one
  run. Patch181's explanation - that SkyrimNet sends each request twice and
  drops one - was wrong; nothing in the logs supported it. Lines now hang
  under the record their words came from, and a run of lines is one
  character's.

## v3.76 patch183
- **Spoken lines are placed by sentence.** A line is filed under the reply
  whose sentence it is; short exclamations - "Ha!", "Oh!", "Hm." - no longer
  land on whichever reply happens to contain those two letters inside a word,
  and a line no reply holds stands beside the clock instead of being guessed
  at.
- **The player's lines, with PTI off, bundle** into one box when they follow
  within ten seconds of each other, rows one after another, instead of a box
  each.
- **Model families read live.** A model of a new architecture (Spark 2.5,
  Nanbeige) now shows its family in the menu even when it was scanned before
  the panel learned to name it.
- **Full window** keeps a small right-hand gap so the terminal's own scrollbar
  is visible.
- **The payload viewer shows structure.** A reply that is JSON (Charbio, and
  any provider that answers in JSON) is shown key by key, with real line
  breaks inside the text and each `{% block %}` on its own line - as
  SkyrimNet's own viewer shows it - and the prose is painted with the same
  colours the Thinking terminal uses: numbers, quotes, names, tags.

## v3.76 patch182
- **Boxes now shrink to fit a narrow terminal.** A record row is columns, not
  prose: it no longer wraps into a jumble; it overflows, and the box scales
  itself down - contents and all - to fit the window. Spoken lines still wrap.
- **Full window** now lands on the bottom row instead of several rows above it.
- **Action rows read as intended.** The text after "Embedded action:" is cyan
  with the usual vocabulary colours over it (it was white - the base colour
  reached only the spaces between the words), the action or intent name wears
  the highlight with a slight glow, and the same on an action row, whose tree
  link glows like every other branch.
- **Box gaps are the same on every side** for single- and multi-row boxes, from
  one shared value.

## v3.76 patch181
- **Fixed: a model of a new architecture had no family in the menu.** The
  panel knew the architecture (the card showed it) but only named the ones in
  its table. An unknown architecture is now named from its own string -
  `spark2_5` reads Spark 2.5 - so no new family is ever blank again.
- **Fixed: spoken lines filed under the reply before the one they came from.**
  SkyrimNet sometimes sends a request twice and drops one; the dropped stream
  left a half-read ghost of the reply in the panel's speech ledger, and the
  placement rule handed lines voiced out of order to that ghost. A dropped
  stream is now recorded (the panel log says the client dropped it mid-reply),
  its ledger entry is closed, and a line goes to the reply that contains its
  words - unspoken occurrences first, the older reply first when both could.

## v3.76 patch180
- **The proxy is a proxy again.** What llama.cpp sends is what SkyrimNet
  receives, byte for byte. Since patch149 every reply chunk of a provider with
  reasoning off - Dialogue, Combat, GM, Meta, UT, AI-Assistant - was rebuilt on
  its way out, and the leading-thought reorder had spread from Dialogue to
  Combat; the official 3.76 rewrote only a Dialogue reply that opened with a
  thought. Both rewrites are now **off by default** and named on Proxy Setup
  (Response wire) for anyone who wants them back.
- **The panel's record-keeping no longer sits on SkyrimNet's clock.** The
  payload file, ledgers and dashboard line are written after the reply's last
  byte has gone out and the connection has closed, not before.
- **A request that fails inside the panel is never silent.** It answers 502
  with the reason and appears on Log > Errors with the port it arrived on;
  before, a handler that failed dropped the socket and left no line anywhere.
- **Fixed: the "previous session ended" line had vanished** from every
  terminal since patch179.
- **Fixed: the "Allocated to…" line flashed** on every card redraw while a
  server was loading.

## v3.76 patch179
- **Fixed: spoken lines landed under the wrong call, or stood on their own.**
  The Proxy terminal placed a spoken line by the clock, and SkyrimNet voices a
  reply's sentences over tens of seconds - some before the call has even
  finished. A line is now placed by its words: the panel keeps every reply
  from its first word, finds the sentence in it, and hangs the line under that
  call with the speaker that call was for - whichever voice SkyrimNet chose to
  say it with. A line the player typed is filed under the PTI call that
  tagged it, first and always - it no longer lands under an NPC reply that
  happened to quote it.
- **Calls with a thought and no speech: the panel now says why.** In this
  session 31 of 43 replies had sentences that never reached the speech door
  at all - SkyrimNet did not ask for them. Once a reply is old enough to be
  sure (two minutes), the record lists the sentences that were never
  requested; open the call's payload to see them under the response, and the
  TTS terminal notes it with a link to the call.
- **The "previous session ended" line is no longer drawn as a box** in any
  terminal.
- **Folder Settings:** every folder title now has its explanation behind the
  ? chip, and "Sweep launcher folder" is a title with a Sweep now button
  under it.

## v3.76 patch178
- **Fixed: a model copied into the models folder was mis-read and stayed
  wrong.** Windows gives a file being copied its full size straight away, so a
  scan that overtook the copy read zeros, kept the first header key and called
  the file a complete "other". The panel now knows a header that ends early is
  an unfinished read, marks the file **TBS** (red, in the menu row) and, on
  Windows, asks whether another program still holds the file open - a file
  still arriving is never counted as failed. The models folder is watched, so
  when the copy finishes and the folder goes quiet the file is re-read and the
  list updates by itself - no restart, no page change.
- **Fixed: the model menu closed while the list refreshed.** An open menu now
  refreshes where it stands, and the card redraw waits until it closes.
- **Fixed: a re-read model kept its old row** because only the number of
  models was compared. The list is compared by what it says.
- **Fixed: the player's spoken line stood on its own** beside the PTI record
  that tagged it, and its emotion tag was shown twice. It now hangs under that
  record like a character's line under its reply, with one tag.
- **The TTS Calibration terminal is boxed per record** instead of one box
  around everything.
- The User Guide's step-7 note is back to showing only while the step is
  still to do.
- **Log > Errors speaks plainly.** Each entry now says what happened and what
  to do, with the raw line underneath; hover a source chip to see where it
  came from. A fault reported by a browser tab still running an older page is
  named as such. A server launch that never came up, and an install whose
  files differ from the release, now appear there too.

## v3.76 patch177
- **Fixed: the terminals could go completely blank** as soon as a record
  carried an embedded action. Nothing was lost - the log was intact and only
  the drawing failed - but no terminal would show anything until the panel was
  restarted.
- **The voice-model list no longer ships.** `tts-model-kinds.json` is a record
  the panel builds on your own machine, like `model-kinds.json`, and the copy
  in every release so far was the developer's - several hundred entries for
  temporary test files that never existed on anyone's PC. It is no longer in
  the release or the update manifest, so an update cannot overwrite your own
  copy, the start-up install check no longer reports the panel's own cache as
  a changed file, and an install that already carries those entries drops them
  the first time the list is read (the panel log says how many went).
- **Fixed: the error notice at the bottom of the page** showed `&lt;-` between
  the function names instead of `<-`.
- patch175 and patch176 were built but never published; everything in them is
  in this release.

## v3.76 patch175
- **A model finishing its copy is picked up the next time you open a model
  menu** - no waiting, no restart.
- **The model menu no longer closes** when the list refreshes underneath it.
- Folder Settings titles are white with the explanation behind the ? chip, as
  intended, and the remaining grey paragraphs are tooltips.
- Records too wide for the terminal now shrink to fit instead of overlapping.
- Panel borders are thinner with the same look; reference chips sit on the
  row's centre line.
- The manual-step note on User Guide step 7 stays visible.

## v3.76 patch174
- **A model that was still being copied when the panel looked at it is now
  read again by itself**, as soon as the copy finishes - no restart needed.
- **Files the panel could not read are marked TBS** so you can tell them from
  files whose headers say nothing.
- Folder Settings finally has its short titles with the explanation behind a
  hover.
- Clicking a reference chip now lands at the top of the record instead of its
  middle.
- Panel borders are half as thick again, same shape.

## v3.76 patch173
- **A model still being copied in is no longer scanned at all** - the panel
  checks the file against the size its own header declares, and reads it once
  it is all there.
- **The model menu no longer closes under you** when the list changes while it
  is open; the refresh waits until you are finished with it.
- Provider names are bold in every terminal, and the payload card's header
  reads its colours from the same place the terminals do.
- Panel borders are a thin bright line with a soft fade either side, and the
  padding is even all round.

## v3.76 patch172
- **Models are remembered one entry per file.** A model that changes is read
  again in place, and one that is deleted is forgotten - the list no longer
  keeps stale copies.
- **A model still being copied in is no longer half-read** and recorded as
  unreadable; it is picked up once the copy finishes.
- **Fixed: opening a server card's model menu could close it again.**
- **Fixed: clicking a [Thinking] or [Speech] chip now takes you there** - it
  was landing on a copy in the terminal you could not see.
- A record too wide for the terminal window shrinks to fit instead of running
  over the row below.
- Provider names are bold; the session separator sits outside the panels.

## v3.76 patch171
- **A model added while the panel is running now reaches the server card menus
  by itself** - the panel tells the page as soon as it finds one.
- Panel borders are a single soft line again instead of two faint ones.

## v3.76 patch170
- **Reference chips now work.** Clicking [Thinking], [Speech] or [Player tags]
  on a record opens that terminal and lights the matching block.
- **The Show link on a long line stays open** instead of closing itself a
  moment later.
- **A model added while the panel is running now appears in the server card
  menus** without a restart.
- Folder Settings is tidier: each path has a short title with its explanation
  behind a hover, like the TTS page.
- Removed a redundant line from the allocator and a redundant instruction from
  Folder Settings.

## v3.76 patch169
- **Fixed: the whole terminal flashed** whenever a single new line arrived.
  Only a genuinely new panel fades in now, and switching terminals no longer
  animates everything at once.

## v3.76 patch168
- **Embedded action rows now match the rows around them**: the branch line
  connects and glows like every other, the icon sits in the same column, and
  a long line wraps under its own text instead of back to the margin.
- Numbers, quotes and other familiar elements inside action rows now take the
  same colours they do everywhere else in the terminal.

## v3.76 patch167
- The embedded-action link mark is accent coloured and glows, like every other
  link.
- Panel borders are drawn half as thick.

## v3.76 patch166
- **A record is one panel again**, with its thought, spoken lines and action
  inside it.
- The Thinking, PTI/PME and Calibration terminals no longer break a single
  record into a panel per paragraph, and their panels take the provider's
  colour like everywhere else.
- Panel edges glow inward as well as out, so the edge reads as a soft border.
- More space between panels.

## v3.76 patch165
- **Fixed: switching to Split View could throw** when the Thinking feed was
  shown.
- Panels are now as wide as their text, and the Thinking, PTI/PME and
  Calibration terminals have them too.
- Panel glows show the provider's colour again - they were falling back to the
  accent whenever timestamps were switched off.
- The TTS terminal now keeps one log per run like the others, so after a
  restart a record's spoken lines come back with it.
- PTI/PME and Calibration open on your last session instead of "nothing yet".
- The embedded-action link mark is coloured and glows.

## v3.76 patch164
- Each panel in the terminals now glows in a colour that says what it belongs
  to: the provider's colour in the Proxy and Thinking terminals, and in the
  TTS terminal the colour of a spoken line or a thought.
- A record and its branches are separate panels sharing one colour, with a
  little more room between records.
- Panels fade in over half a second, as does a server card's allocation line.

## v3.76 patch163
- **Each log entry now sits in its own panel** in the TTS, Thinking, PTI/PME
  and Calibration terminals, so you can see where one report ends and the
  next begins.
- The Proxy terminal groups a record with everything belonging to it - its
  thought, its spoken lines, its action - and alternates the shading between
  records.
- The payload card shows the token counts as one field: input, output, and
  the reasoning tokens in brackets when the call used any.

## v3.76 patch162
- Terminal colours are defined in one place again: the newer line types added
  over the last few patches now take their colours from the same table as
  everything else, so a theme change reaches all of them.

## v3.76 patch161
- **Added a provider for the SkyrimNet-Relationships plugin.** It arrives
  unallocated like any other - place it on a server from Live Network, and
  point the mod's `snrom_background` variant at it.

## v3.76 patch160
- **Fixed: a server could refuse to start** when Built-in MTP was still
  selected from a previous model that had one. The panel now warns on the
  card and launches without it instead of letting the server fail.
- Fixed a doubled emotion tag on player lines.

## v3.76 patch159
- **Voice tags now survive on the first line after a speaker switch.** The
  panel used to strip them and warm the voice with a hidden chunk first; that
  cost about 1.5 seconds per switch, removed the performance from exactly the
  lines that carry it, and could drop a line entirely.
- A line with no words but a sound - a laugh, a sigh - is spoken instead of
  being skipped.
- Voice Prime is gone from TTS settings; there is nothing left to switch.

## v3.76 patch158
- **Spoken chunks are now synthesised in the order they arrive**, so a reply's
  sentences appear in the terminal in the order they are said. Previously
  whichever chunk was quickest to prepare went first.
- **Speech timings are honest again**: a chunk's time no longer includes
  waiting behind another chunk, which had been skewing the panel's own
  speed calibration.
- The TTS terminal now records every speech request the moment it arrives,
  and says what asked for it - SkyrimNet, the panel's thought audio, or a
  click in the terminal.

## v3.76 patch157
- **Fixed: the thinking label changed when a server started.** llama.cpp does
  not report whether a template has a thinking switch, so the panel now uses
  the model file's own answer whether or not the server is running.
- **Fixed: settings could silently fail to save** when two changes landed at
  once - the panel wrote both to the same scratch file and the second lost.

## v3.76 patch156
- **If spoken lines are not being shown in a terminal, it now says so** and
  names the switch that shows them, instead of simply looking empty.
- Embedded actions show what was chosen again - *Embedded action: Combat
  (intent: ...)* - with the label plain and the action in cyan.

## v3.76 patch155
- **Fixed: every server card claimed its model had no thinking switch.** The
  panel was asking llama.cpp for a detail it no longer reports, and treating
  the silence as a "no". It now reads what the server does report, and falls
  back to the model file's own answer when the server says nothing.
- The proxy terminal shows every voice tag a reply carried, including the one
  that opened it.
- The terminal remembers 150 request/reply pairs instead of 80, **and keeps
  them across a restart** - so a line carried over from your last session can
  still be opened. Note: this writes the full request, including the character
  card and system prompt, into a file in your log folder. Deleting the log
  folder removes them.
- The "previous session ended" divider is now shown as a highlighted notice.

## v3.76 patch154
- **Fixed: an NPC could speak the "4.09x" speed reading of their own thought**
  when the thought was clicked to replay - and would re-record it each time.
- The proxy terminal now shows each voice tag by the model's own word
  (*contemplation*, not *thoughtful*), so the line matches the payload.
- Embedded actions read simply as *Embedded action: Name*.
- The payload card shows the model name in the accent colour and marks a
  streaming reply with a wireless symbol.

## v3.76 patch153
- **The terminals remember where you left off** - the last session's lines are
  above this one's, separated by a rule.
- The version button's tooltip now tells you which version you are running and
  whether an update is waiting.
- Main guide: the last step shows the **Launch LLM** button itself, ready to
  press, and every step's tasks are in bold.
- Toggle switches are identical everywhere; the Sampler Guide's value line no
  longer curves into its own corner.

## v3.76 patch152
- **The startup screen now counts as it goes** instead of jumping - it follows
  the model scan in real time.
- The model search field is the menu's dark grey rather than see-through, and
  the menu is exactly as wide as the selector it belongs to.
- TTS guide: the model repository is linked at the top, and every download
  command names your own models folder, wherever you installed PandorumLLM.

## v3.76 patch151
- **A model added to your models folder is picked up straight away.** The
  panel checks the folder when you open a model menu and when you open the
  Servers or TTS page, instead of waiting out a five-minute cache.
- If a model file cannot be read, the panel now tries five times, then stops
  and tells you which file, what was missing and why - rather than retrying
  it silently for ever.

## v3.76 patch150
- **Embedded actions show in the proxy terminal**: when SkyrimNet's embedded
  action evaluation has the dialogue model choose an action with its line, it
  appears under that reply as *Embedded action: who > Action (…)*.
- The TTS guide is one **audio.cpp** guide now, with every voice model listed;
  the separate Higgs page is folded into it.
- Main guide: each step's tasks are bullet lines centred in the box, and
  hovering a box no longer loses its tooltip or restarts its glow.

## v3.76 patch149
- **Combat lines are voiced properly.** A short combat line lost its speaker,
  switched voices and waited on priming before it could be spoken; combat
  replies now register their speaker, keep their voice, and their spoken
  lines sit under their own row in the proxy terminal.
- **Reasoning a model produces with thinking off** is now kept out of what
  SkyrimNet receives and shown as reasoning in the payload view; such a model
  is marked *thinking: trained*.
- Every model file is re-read once for its reasoning facts.

## v3.76 patch148
- **The model picker's search field is reworked**: it appears above the bar
  only while the menu is open, in the menu's own dark style, with a white
  magnifying glass, and it takes focus when the menu opens.
- Fixed: voiced thoughts never showed their realtime ratio in the proxy
  terminal.
- Fixed: spoken lines in the proxy terminal could appear out of order under
  the wrong reply.
- The payload card now names the model that generated the reply.

## v3.76 patch147
- **The user guides catch up with the panel.** Main guide: step 1 is now
  *Install llama.cpp, check folder paths*, step 2 starts with the 1 or 2 PC
  choice, every step has a hover tip, and clicking a step highlights what to
  press - including the install button, the PC GPUs control, and every box on
  Live Network. The Servers page no longer jumps to the middle when a step
  opens it.
- **TTS guide:** a new *audio.cpp* tab explains the one-click install (with
  the manual fallback and the folder it goes to), and lists every voice model
  the panel can run on it, with a download line for each. The Higgs page
  points at it instead of repeating it; the proxy page names its model.

## v3.76 patch146
- **A search field above every model picker** on the server cards (model,
  vision projector, draft model). Type to narrow the list to matching names;
  the menu shrinks to fit; Enter takes the first match. For long models
  folders.

## v3.76 patch145
- Internal: the release checks were made independent of one another - each
  now starts from a clean panel, and any that leaves files behind is named.
  No user-facing changes.

## v3.76 patch144
- **The blue [--flag] buttons now open the Sampler Guide's card right where
  you are**, as a pop-up - the same card, live - with a *Sampler Guide Page*
  button at its foot for the full page. Click outside or press Escape to close.
- Fixed: the four DRY entries in the Sampler Guide (base, allowed length,
  penalty last-n, sequence breakers) showed no range line.

## v3.76 patch143
- **Thinking: trained.** When a model returns reasoning although thinking was
  switched off, the panel now notices, marks that model, and shows a yellow
  warning beside the Model selector, the Reasoning setting and the provider's
  Thinking toggle. The mark is kept for that file and never guessed from
  reply length.
- The reasoning a server returns is now captured in the payload view whether
  or not thinking was on.
- Shorter tooltips on the thinking chips and the draft-model mismatch; the
  Sampler Guide explains the settings.

## v3.76 patch142
- Internal: the release checks now look for the kind of lock re-entrancy
  that once froze the panel, and for two other faults the checks themselves
  could hide. No user-facing changes.

## v3.76 patch141
- **Failed updates and failed startup steps now appear on the Log page** as
  errors, with stable codes (UPD-E01 to UPD-E10) that can be quoted.
- Internal: the checking system that guards each release was strengthened -
  it can no longer be stopped early by the faults it tests for, it cleans up
  after itself, and it now catches a misspelt name before a user does.

## v3.76 patch140
- **Remote Access shows the panel's version status** - green when it is the
  newest release, yellow when an update is available - without being able to
  start an update; that stays on the host.
- The Permission Tree describes the updater: what it fetches, what it checks,
  where it writes, and what a remote viewer is shown.
- Only the latest rollback backup is kept after an update; older ones no
  longer pile up in logs.

## v3.76 patch139
- **The update now clears up after itself**: the files it replaced, its
  download folder, and any leftovers from an earlier attempt - shown as a
  final stage, *Clearing the old files*. The backup of your previous version
  is kept.
- Fixed: each update left `.old` copies of the replaced files in the folder.
- The update log now names the files it wrote.

## v3.76 patch138
- **Fixed: the update window could stay at "Closing the panel" after a
  successful update.** Each stage of the swap is now reported from what the
  update actually recorded, and the window recognises the restarted panel
  by what it serves rather than by having watched it go down.
- Fixed: updating to a release older than this one could be reported as
  failed when it had succeeded.

## v3.76 patch137
- **Fixed: updating opened a second panel tab.** The panel restarts in place
  now and the page you started from reconnects.
- **The update window stays open when it finishes** and reports the result:
  which stages completed, and *Update successful* or *Update failed* with the
  stage it failed at and why. Done reloads the page.

## v3.76 patch136
- **The version button says "Checking for updates" while it asks GitHub**,
  and clicking it during that time explains what it is doing instead of
  showing nothing.
- The update-test build now says which patch it was made from, in its header
  and in its filename.

## v3.76 patch135
- **Updating is one press.** Update downloads, checks, stages, closes the
  panel, swaps the files and restarts it without asking again; the progress
  bar runs through every stage and each stage is listed as it completes, in
  the same style as the startup screen.
- **Fixed: the browser asked "are you sure you want to leave?"** after an
  update, or on first start. That prompt is now only for closing the tab.
- **Fixed: on a fresh install, the welcome screen appeared before the panel had
  finished starting.** It now waits.
- Fixed: brief terminal windows flashing while an update ran.
- Fixed: the update window's buttons flickering on hover.

## v3.76 patch134
- **Fixed: the Update button in the update window did nothing**, and its
  buttons flickered on hover. The window was being redrawn from scratch every
  half second; it now updates in place like the rest of the panel.
- **The whole update now runs inside that one window** - download, checks,
  closing, swapping and restarting are each shown as they happen, and the
  page reconnects when the panel is back.

## v3.76 patch133
- **The update window is now the same as the llama.cpp and audio.cpp ones**:
  what is installed, what is available, the file and its size, the published
  checksum, then Update and Cancel - and it updates itself as the download
  progresses, with each check on its own line.
- **Any published release can now be installed**, not only ones carrying a
  file manifest. Every file is still verified against the checked archive.
- Fixed: a release without a manifest was reported in a way that looked like
  a failed checksum.

## v3.76 patch132
- **Fixed: the updater could not find the checksum published with a release**,
  and reported that the release had none. It now recognises the checksum
  whatever it is named.
- **Fixed: a failed update check was remembered for six hours**, so trying
  again changed nothing. It is now forgotten after a minute, and the panel says
  what actually went wrong.
- **The update window now matches the llama.cpp and audio.cpp ones** - same
  layout and buttons - and shows the published checksum it is verifying
  against.
- Everything the updater does is written to **logs/update.log**.

## v3.76 patch131
- **Fixed: thinking settings had no effect.** Servers were started without
  `--jinja`, which llama.cpp requires before a model's own chat template - and
  therefore its thinking switch - is used at all. Every server now starts with
  it.
- **The sampler order is now stated explicitly** rather than left to
  llama.cpp's default, so it cannot change under you when llama.cpp updates.
- **Opening a completed generation now shows the samplers it actually used**,
  colour-coded by who chose each one: the server, SkyrimNet, or this panel.
- The Sampler Guide covers all five DRY settings; it previously described only
  the multiplier.

## v3.76 patch130
- **Fixed: after updating, the panel would report its own files as changed.**
  The update was not installing the file list it checks itself against.

## v3.76 patch129
- **Fixed: no models would load at all.** A model file that states a setting
  once per layer, rather than once for the whole model, stopped the folder
  scan dead - and with it every model in the folder. Several such settings
  were affected.
- **The startup screen now shows the folder scan counting up**, so a long
  scan reads as progress rather than a pause.

## v3.76 patch128
- **Fixed: the model list could fail to load and never recover.** The panel
  now keeps asking, and the list appears as soon as the folder has been read.
- **The panel no longer says a model is "not in models folder" while it is
  still reading that folder.**
- **New: PandorumLLM shows what it is doing while it starts up**, step by
  step, and opens when it is genuinely ready to use. A step that fails says so
  rather than leaving you to notice something missing later.
- Reading the models folder no longer holds up the page.
- **Fixed: the model menu could not be dragged taller than a set height**, and
  a short menu was given a large empty box.
- Higgs v3 is now called **Higgs TTS 3**, which is what its developer calls it.

## v3.76 patch127
- **PandorumLLM can now update itself.** When a newer release exists, the
  version button says *Click to Update*; the panel downloads it, checks it
  against the published checksum, and shows you what it found before anything
  on your disk is touched.
- Applying it closes the panel, replaces the files, and starts it again - the
  page you are looking at waits and reconnects on its own.
- **Every file is checked as it is written**, and the ones being replaced are
  kept in logs. If anything at all does not match, the update stops and puts
  everything back: you are never left between versions.
- The panel checks its own files against the release manifest at start, so you
  can see that an update landed correctly.

## v3.76 patch126
- **Fixed: some model details were never read.** Settings that a model states
  per layer rather than once were skipped entirely, so several families showed
  no attention ratio and could not be matched to their drafting heads.
- **Fixed: models whose thinking cannot be switched off showed no label.** The
  panel only recognised one way of writing it; it now recognises the common
  conventions, including models that take a reasoning *level* rather than an
  on/off switch.
- **The Sampler Guide reads better.** Entries are broken into paragraphs and
  lists instead of one block, and no longer name particular model makers.
- **Reasoning strength** now has its own guide entry, reachable from the card.
- **A dropdown opens at whatever is already chosen**, centred in the list, and
  can be dragged taller.

## v3.76 patch125
- **Fixed: the wrong values corrected in patch122 were never actually
  refreshed.** The remembered model details are re-read once on the next start,
  which takes a few seconds and then settles.
- The *dense* label is now white, and a model file that is itself a prediction
  head is shown in magenta.

## v3.76 patch124
- **Vision projectors and drafting heads are matched to their model far more
  often.** Where two model families share the same size, the panel now uses
  which folder the file is kept in to tell them apart. Still shown with a ~,
  since it is inferred.
- A drafting head whose model cannot be identified now shows what kind of head
  it is, instead of nothing.
- **Built-in MTP is now yellow and vision is cyan.**

## v3.76 patch123
- **Fixed: the panel would not start after patch122.** A message added in
  patch122 was written at a moment when the panel was not able to write
  anything, and it stopped there instead of finishing startup. The message is
  now sent once startup is safely past that point.

## v3.76 patch122
- **Fixed: some values read from model files were stored wrongly.** Decimal
  settings such as the rope base were recorded as meaningless whole numbers.
  Your remembered model details are refreshed once on the next start.
- **The panel now says what it did with its remembered model details** on
  every start - whether they were current, or how many had to be re-read.
- **Fixed: drafting heads were not matched to the model they belong with.**
  The match required a figure that is always smaller on a head than on its
  model, so it never succeeded.
- **Clearer labels:** a model with a built-in prediction head reads *built-in
  MTP*; a file that is one reads *MTP*. Model names no longer repeat what the
  labels beside them already say.

## v3.76 patch121
- **Vision projectors are now matched to the model they belong with**, by the
  width they project into. Shown with a ~ to mark it as inferred.
- **One label for models that predict ahead.** There were four - MTP, MTP
  drafter, DFlash, DSpark - where one is enough; whether the file is a model
  carrying a head or the head itself is already shown beside it.

## v3.76 patch120
- **Files that are not language models are now recognised as such.** An image
  model sitting in your models folder was being offered as something to load;
  it is now marked **other**. A model, projector or drafting head is never
  mistaken for one.
- **A drafting head or projector with no family of its own can now be matched
  to the model it belongs with**, by comparing several of the figures both
  declare. The match is shown with a ~ to mark it as inferred, and is only
  offered when exactly one model family fits - matching on size alone is not
  enough and is never used on its own.

## v3.76 patch119
- **Fixed: models with a built-in MTP head were also labelled as drafters.**
  They now show only their own built-in label.
- **Fixed: some drafting models were offered as ordinary language models**, and
  were labelled dense as well.
- **Fixed: mixture-of-experts variants lost their family name** - an LFM2 MoE
  model showed no family at all.
- **Fixed: a model that does not fit its server showed two warning symbols.**
- Where a file's architecture only says what it does - a drafter, a projector -
  no family label is shown, since the row already says what it is.

## v3.76 patch118
- **The architecture label now shows the family and nothing else** - *Gemma 4*,
  not "Gemma 4 assistant (MTP drafter)", since what the file is for is already
  shown after its name. A Mistral reads *Mistral* rather than "Llama family",
  and a model whose architecture the panel does not recognise shows no label at
  all rather than a raw tag.
- **Fixed: drafting heads and vision projectors were being called dense**, which
  describes how a model is built and does not apply to a part of one.
- A model that does not fit its server now shows a warning symbol with the
  reason as a tooltip, instead of a line of text in the menu.
- The chosen model is shown by name alone; the labels stay in the menu.

## v3.76 patch117
- **Fixed: the model menu labels were wrong.** No architecture was shown at
  all, vision projectors were labelled as language models, and a model with a
  built-in MTP head lost its own label to it. Each file now shows its
  architecture, what it is, and what it carries - including **dense** and
  **MoE**.
- **New setting: Drop the thinking primer.** Some models open every answer with
  a think tag written into their own chat template, so no setting can turn it
  off. This writes a copy of that template without the tag and points
  llama.cpp at the copy. Your model file is never changed, and nothing happens
  for a model that does not do this. Off by default.

## v3.76 patch116
- **The model list on a server card now shows what each file is** - its
  architecture before the name, and what it is for after it: LLM, vision,
  draft or MTP.
- **Fixed: models were wrongly labelled as having built-in reasoning.** The
  label was applied to a whole model family, including files with no reasoning
  mode at all. The panel now reads each model's own chat template, and where a
  server is running it asks llama.cpp directly - which is the only way to know
  whether the reasoning switch actually works.
- The card also reports the expert counts of a mixture-of-experts model, its
  grouped-query attention ratio, its rope scaling and its full-attention
  pattern - each one a setting llama.cpp acts on.

## v3.76 patch115
- **The menu field is now the same dark as the menu itself**, as intended.
- **Better support for LFM2 / LFM2.5 models.** The panel reads the model file's
  own layers and reports how many of them use attention - for example *3/16
  layers attend* - which is why these models need so little memory for their
  context. Files that use a mixture of experts are recognised too. This is read
  from any model built that way, not from a list of names.

## v3.76 patch114
- **The menu and its field are now the same colour** - they had been set
  separately and kept falling out of step.
- Menu labels sit properly centred in their own boxes.
- **The proxy MOSS entry is a different model from the audio.cpp one** and now
  says so: it is listed as *Moss TTS 1.5 Delay*, the larger model the separate
  MOSS server loads. Its size is read from the publisher's own file listing,
  with an approximate figure shown until that has been read.

## v3.76 patch113
- **Fixed: the download sizes in the TTS menu were missing** until a voice was
  selected. They are now read when the panel starts, so they are there from the
  first time the menu is opened.
- The menu labels lost their boxes, sit properly on the middle of each row and
  carry a soft glow; voice names are bold.
- **Every dropdown on the page now uses the same darker field**, so a menu is
  easy to tell apart from the panel behind it.

## v3.76 patch112
- **Fixed: the TTS menu showed empty rows** and the rows were too short.
- **New start/stop button beside the TTS chooser** - a triangle to start the
  voice server, a square to stop it, doing exactly what the Launch TTS button
  in the header does.
- "Revert to default TTS settings" is now the same circular arrow used by the
  folder rows, with its wording as a tooltip.

## v3.76 patch111
- **The TTS chooser is back on the panel's own menu component**, so it behaves
  like every other dropdown here.
- **Each voice now shows what runs it** - audio.cpp or proxy - **its download
  size, and a WIP mark** where one applies, in that order.
- MOSS appears twice under one name, once for each engine that can run it,
  listed together.
- The chooser sits on a darker field so it reads as a control.

## v3.76 patch110
- **Fixed: the new TTS chooser did not open when clicked.**
- The chooser is now half its previous width, sits on its own dark background
  so it reads as a control rather than part of the page, and its arrow is
  larger and lights up with the title when you hover it.
- **Fixed: hovering the TTS name drew a rectangle around it** instead of
  lighting the letters.

## v3.76 patch109
- **The TTS chooser now shows a size for every voice** - the range from the
  smallest to the largest model it can be downloaded as, read from the real
  files rather than a fixed figure.
- **Voices still being worked on are marked WIP.**
- Titles no longer repeat the engine name, Higgs is now **Higgs v3**, Moss is
  **Moss TTS 1.5 Local**, and Chatterbox is no longer labelled "voice clone
  only" in its own name.

## v3.76 patch108
- **Fixed: long lines in the voice terminal could print on top of the line
  below.** A very long record - the banned tag list, most often - was given a
  single line's height however many lines it actually needed.
- **Long records now fold.** The label stays on its own line with a **Show**
  button next to it, and the rest appears only when you ask for it. Ordinary
  lines are unchanged.

## v3.76 patch107
- **Support for LiquidAI LFM2 / LFM2.5 models** (such as Shruti-Soft-2.6B). The
  server card now explains what this family brings: only some of its layers use
  attention, so its KV cache is unusually small for the context and the rest of
  its state appears as Recurrent state in the allocation report.
- **Two new server settings under Context and cache:** *Unified KV cache* and
  *KV limit per slot*, which decide how the cache is shared between a server's
  parallel slots. They matter most for models like these, where only some
  layers use attention.
- **These models reason because they were trained to, not because a switch turns
  it on** - so a provider with Thinking switched off may still see the model
  think. The card now says so, and points at the control that does work:
  Generation > Reasoning budget set to a small positive number with a budget
  message. Zero is the wrong value there and the card warns about it.

## v3.76 patch106
- **Fixed: some TTS voices refused every line.** DramaBox and DotTTS do not
  accept a temperature setting, and the panel was sending one to every voice
  alike, so every line came back as an error. The panel now notices when a
  voice says it cannot use a setting, leaves it out, and asks again - so the
  line is still spoken, and later lines are sent correctly first time.

## v3.76 patch105
- **Fixed: some TTS models would not start.** The panel was sending a setting
  no current audio.cpp accepts, and the automatic retry that worked around it
  only had time to act on small models - a large pack simply failed to launch.
  The setting is no longer sent, and the "Cached Voices" option that fed it has
  been removed, as it could not reach the engine at all.
- **Fixed: short lines could be cut off before they finished.** A line costs a
  fixed amount before it costs anything per character, which the token limit did
  not allow for; it now does, and will also widen itself if this machine is
  measured to need more.
- **Fixed: MOSS could fall silent for half a minute between sentences.** A limit
  of ten seconds was applied to each pause in a line rather than to the line, so
  four pauses meant forty seconds. The limit now covers the whole line.

## v3.76 patch104
- **Fixed: after a llama.cpp update finished, the TTS page kept saying one was
  running** and would not show the audio.cpp status or the chosen model until
  the finished update was dismissed on Folder Settings.
- **A finished update or install now clears itself when you leave its page.**
  The Dismiss button is still there if you want it gone sooner.

## v3.76 patch103
- **audio.cpp 0.7 is now the minimum**, and the "Audio.cpp Build" setting has
  been removed. Releases up to 0.6 shipped separate Balanced and Fast builds to
  choose between; 0.7 dropped them for a single build per CUDA line, so there
  was nothing left to choose. An older install is still detected and reported
  as too old rather than simply not found.
- The manual install steps now name the archives 0.7 actually publishes - they
  still described the 0.6 ones.

## v3.76 patch102
- **The "this is a read-only view" pop-up is gone.** A remote view refreshes
  constantly, so the message could arrive faster than it could be dismissed.
  The header already carries a read-only badge and the controls that need the
  host PC are not shown at all, which says the same thing without interrupting
  anyone.

## v3.76 patch101
- **Fixed: emotion tags were printed in front of spoken dialogue.** They are
  now shown as a stage direction - *amused*, *disgusted* - as was always
  intended. The tag itself always reached the voice engine correctly; only the
  text on screen was wrong.
- **Fixed: thoughts could carry an emotion tag.** A thought is never spoken
  with a mood, and no longer shows or speaks one.
- **Fixed: a remote view warned "this is a read-only view" over and over**, so
  often it could not be dismissed. The allocation report was asking for
  something a remote view is not allowed to request; reading a server's own log
  is now permitted from a remote view, as it always should have been.

## v3.76 patch100
- **The allocation report reacts immediately again.** Launching servers from
  the main Launch button left each card showing the previous run's figures
  until something else disturbed it; the card is now cleared as the launch
  starts, and the report follows the load closely enough to show each buffer
  arriving rather than jumping from the weights to the finished total.
- **"Free at load" and "Free before drafter" have moved to the allocator log.**
  They are not allocations, so they do not belong in a list of allocations -
  and they were being labelled as though they had been allocated to VRAM.
  `logs/allocator.log` now records the headroom each launch had, both before it
  began and when the draft model's turn came.

## v3.76 patch99
- **Fixed: Compute could be over-reported.** When a draft model re-reserved its
  graph after a vision model had loaded, the same buffer was counted twice.
- **New: "Free at load" now works**, showing what was free on the card before
  the model loaded and how large the card is - and a new **"Free before
  drafter"** line showing what was left when the draft model's turn came, which
  is the figure that decides whether it fits.
- The allocation report's header now puts the launch date and time on its own
  line beneath the model name.
- **Fixed: values kept pulsing** after everything had finished loading. Each
  figure now flashes once, when it arrives.
- **Fixed: hovering a collapsed section heading** lit only the arrow instead of
  the whole heading, and the arrow sat slightly high.
- The engine install windows and the other pop-up dialogs now match the rest of
  the panel - the same dark background, with a soft black glow in place of the
  grey border.

## v3.76 patch98
- **Fixed: the allocation report stopped at the weights and went no further.**
  The panel could not send a report once the server log reached its first
  compute buffer, so the browser received nothing at all and each card kept
  whatever it had read moments earlier. Small models, which reach that point in
  a few seconds, showed no report at all. This has been the cause since
  patch86.

## v3.76 patch97
- **Fixed: allocation reports stopped filling in when several servers were
  launched together.** The panel was refusing its own page's requests - it
  accepted only a handful of simultaneous connections, and launching a fleet
  from the Launch button asked for one per server at the same moment. The
  browser reported a network error and each card kept whatever it had read
  first.
- **The report now asks for every server in a single request**, so a fleet of
  twenty costs no more than a single server, and once every log has finished
  it asks for nothing at all.

## v3.76 patch96
- **An allocation report that stops filling in now says why.** If the panel
  cannot find the log, is waiting for the current launch's log to appear, or
  the request fails, the card says so instead of quietly keeping its last
  figures. A working report shows none of this.

## v3.76 patch95
- **Fixed: the Draftless speculation settings only appeared when a draft model
  was selected.** N-gram speculation needs no draft model at all - that is the
  point of it - and the settings are now always shown.
- **The allocation report now follows the server log rather than a timer.** It
  keeps reading until the log itself says the load has finished, however long
  that takes - a large model on a slow disk is treated no differently from a
  small one - and each reading adds to what is known instead of replacing it,
  so a reading taken mid-load can no longer blank out figures already shown.
- A report that is still incomplete says which log file it is reading and when
  the panel last answered, so a stuck one says where to look.

## v3.76 patch94
- **The allocation report no longer decides when to stop refreshing.** Three
  different rules for that have now each left a card stuck half-filled; the
  report simply refreshes every server continuously, which costs a fraction of
  what the rest of the page already polls.
- **The fold arrow is centred by its heading** rather than positioning itself,
  which is the third and last attempt at getting it to sit straight.

## v3.76 patch93
- **The allocation report keeps updating until it is actually complete.** It
  could stop as soon as the server reported itself ready, which happens in the
  same moment the last buffers are recorded - so a report read a fraction
  earlier stayed half filled.
- **A half-filled report now says which log file it is reading and how long ago
  that file was written.** The line disappears as soon as the figures arrive.
  If a report stays short, that line says where to look.

## v3.76 patch92
- **Fixed: the allocation report could be blank, frozen, or silent about a
  failure.** It was only asked for while a server was reported as serving -
  which is not the case while it is still loading, nor after it has exited. A
  server that failed to launch showed its buffers with no sign of the failure;
  one that was still loading showed nothing at all. Every server is now asked
  until its report is complete.
- **The fold arrow is drawn properly** and no longer pushes its heading around.
- **New: Collapse all / Expand all on each server card**, just above the first
  section, working on that card alone.

## v3.76 patch91
- **The fold arrow sits properly on its heading again.** Making it larger in
  patch90 also made the heading row taller and pushed the text along; it is now
  drawn large without affecting the layout at all.
- **The DRY sampler window matches the rest of the panel** - the same dark
  background as everything else, with a soft black glow in place of the grey
  border.

## v3.76 patch90
- **Speculative Decoding and Draftless speculation now fold too.** They were
  the only two sections that would not, because they are built separately from
  the rest of the card; they use the same heading as every other group now.
- **Draftless speculation now sits above Speculative Decoding.**
- **The fold arrow is much larger** and easier to aim at.
- **Folding is per card.** Collapsing CPU on one server no longer collapses it
  on the others, so cards can still be compared side by side.

## v3.76 patch89
- **Server card sections now fold away.** Click any group heading - GPU,
  Context and cache, CPU and the rest - to collapse or expand it. The arrow
  beside the heading shows which way it will go, and your choice is remembered
  across restarts.
- **The DRY sampler on a provider now exposes all four of its settings.** The
  chip was `--dry-multiplier` alone; clicking it now opens a small panel with
  the multiplier, `--dry-base`, `--dry-allowed-length` and
  `--dry-penalty-last-n`, each with its own field. Leave one blank and it is
  not sent. Click OK, press Enter, or simply click away to apply.

## v3.76 patch88
- **Fixed: the allocation report could stop half way** - showing the weights
  with dashes for everything else - and stay that way however you navigated,
  disappearing entirely after a browser refresh. It was only refreshed while
  the Servers page was on screen and while the panel still considered the
  launch to be in progress; a server answers its port within a second, long
  before the last buffer is allocated. The report now refreshes on its own,
  until the log says loading has finished.
- **The allocator log no longer implies a card is empty** when the driver
  simply does not report per-process memory - which is the case for GeForce
  cards on Windows.

## v3.76 patch87
- **Fixed: the allocation report flashed once and disappeared** when a server
  finished launching. The report is redrawn as part of the card, and it was
  being put back only after a round-trip to the panel - so a card redraw
  arriving first threw it away. It is now restored the instant the card is
  drawn, from the figures already in hand, and stays put.

## v3.76 patch86
- **New: an allocator log.** Every launch appends a record to
  `logs/allocator.log` listing each buffer the engine allocated and the total
  it asked for, then what NVIDIA reports is actually resident on that card,
  process by process. If the two disagree, the log says so - that difference is
  memory which did not end up in dedicated VRAM.
- **And a summary of every card on each launch**: how much it holds, how much
  is in use, which of your servers are allocated to it, and a warning when more
  is in use than the card has.
- **Fixed: the compute buffer could be counted twice.** A reserve line repeated
  with the same size was added twice; it is now counted once.
- **Fixed: the report header printed the word "vhdr"** instead of styling the
  line.

## v3.76 patch85
- **The allocation report now names the buffer that ran out.** An out-of-memory
  failure is marked on the row that actually failed, with the size it asked
  for, instead of on whichever row the report was next expecting - a failed
  SWA cache used to be reported as Compute.
- **A launch that fails for want of memory without saying so is now caught.**
  When the GPU reports no free memory and the next thing to load fails, the
  report says the card was full - previously that failure was attributed to
  nothing at all, because llama.cpp's message never mentions memory.
- **The report is headed with the model it loaded and when it was launched**,
  so it is clear which run the figures describe.

## v3.76 patch84
- **The Draftless speculation rows now match the rest of the card**: the same
  label styling, working reference links on every flag, and dropdowns the same
  width as the number fields beside them.
- The N-gram template row no longer shows a flag - it does not write one; it
  fills the three settings below it.
- Its first option is now called **User specified**.

## v3.76 patch83
- **Server card rows are evenly spaced again.** Everything above the CPU block
  was drawn with wider gaps than everything below it, which was never intended.
- **The n-gram settings now sit under Speculative Decoding**, where they
  belong, under the heading **Draftless speculation** - they are speculation
  that needs no draft model at all.
- **New: an N-gram template.** Choose Dense or MoE and the three ngram-mod
  numbers are filled in and locked to values suited to that kind of model;
  choose User and they are yours again, exactly as you left them. Only the
  settings the chosen method actually takes are shown.
- Hand-edited launchers keep their n-gram settings: the panel now reads them
  back out of the launcher the same way it writes them.

## v3.76 patch82
- **The installer window now says clearly whether a build can be verified.** A
  green tick and the SHA-256 the download will be checked against, or a red
  cross when the release publishes none - and in that case the install button
  is hidden, with a note to use the releases link and unpack it yourself.
- **New: n-gram speculative decoding on the server card.** It drafts from what
  the model has already written, with no draft model at all, and runs
  *alongside* an MTP head rather than instead of it - the card writes
  `--spec-type draft-mtp,ngram-mod`. Types offered: ngram-mod, ngram-simple,
  ngram-map-k, ngram-map-k4v and ngram-cache, each with the settings it
  actually takes.
- Suggested starting points, from llama.cpp's own notes: ngram-mod at its
  defaults (24 / 48 / 64) for mixture-of-experts models such as Qwen 3.8 27B or
  Muse Glimmer 30B, and lower minimum and maximum draft lengths for dense
  models such as the Gemma 4 range. Small lookup lengths are not recommended.

## v3.76 patch81
- **Fixed: patch79 could replace folder paths you had already chosen.** The
  folder defaults added in patch79 were being applied on every start, so an
  existing llama.cpp, models or launcher folder was overwritten with the
  panel's own the first time it loaded. They are defaults now, as intended: if
  you have set a folder yourself it is kept, marked as yours, and left alone.
- **And it repairs what patch79 already changed.** Your server cards name their
  model files and launcher scripts by full path, so the real folders are read
  back from them and restored. Folders nothing else refers to - the log and
  provider YAML folders - cannot be recovered this way and may need setting
  again.

## v3.76 patch80
- **New: auto-sort for your models folder**, under the models path on Folder
  Settings. It files each model by what its header says - kind, family and size
  - so `gemma-4-31B-it-scotoma-2.gguf` goes to `LLM\Gemma4\31B`, a drafter to
  `MTP\...` and a vision projector to `Vision\...`. The filename is never used
  to decide.
- **Nothing moves until you have seen the plan.** "Show what would move" lists
  every file, where it would go, and which server cards point at it. Models
  whose headers do not say what they are stay exactly where they are, and a
  file that would land on top of another is left alone and reported.
- **Server cards follow their models.** Any card naming a file that moves has
  its model, vision or draft path updated in the same step.

## v3.76 patch79
- **Every folder the panel uses now has a home inside PandorumLLM** -
  `llama.cpp`, `models`, `launchers`, `logs`, `providerYAML` and the four TTS
  folders - and each is locked to it, so a stray edit cannot send the panel
  looking somewhere else.
- **A padlock beside each folder unlocks it.** Clicking it asks first and
  explains that nothing is moved or deleted; while it is open the field is
  editable and the panel leaves your path alone. Clicking it again locks the
  folder where it currently is, without reverting.
- **A revert arrow beside the padlock puts a folder back.** It shows where the
  folder is now, where the panel would put it, and every path you have used
  before - up to a hundred, scrollable - and you can pick any of them. The path
  you are leaving is remembered first, so changing your mind is one click.

## v3.76 patch78
- **When an install fails, the panel now tells you how to do it by hand.** The
  download address appears with the error - click it to copy - along with what
  to do: download it, put it in the folder, point the panel at that folder. It
  shows the address for whatever failed, whether that was audio.cpp, llama.cpp
  or a TTS model.
- **"Local Voice Clips" and "Sample Vault" were the same folder** and are now
  one setting, called Sample Vault: your voice clips, and the repaired copies
  the panel makes of them.
- The audio.cpp and TTS model labels are finally the same width.

## v3.76 patch77
- **Downloads are now verified.** Every file the panel fetches - both engines
  and every TTS model - is checked against the SHA-256 its publisher states,
  and deleted rather than unpacked if it does not match.
- **The panel will only download from github.com and huggingface.co**, over
  HTTPS, and refuses to follow a redirect to anywhere else.
- **The installer windows show what they will download**: the exact filename,
  its size, and the SHA-256 it will be checked against, so you can compare
  them with the release page before pressing anything.
- **llama.cpp installs inside PandorumLLM** when you have not chosen a folder,
  instead of a fixed path elsewhere on your C: drive. A folder you chose
  yourself is used exactly as before.
- The second confirmation window when updating llama.cpp is gone - the CUDA
  chooser already asks.
- **The TTS model line lists the quantisations you have** - "Present: BF16,
  Q8_0, Q4_0", largest first - rather than repeating the model name.
- The Permissions tree now states all of this.

## v3.76 patch76
- **The status lines update the moment an install finishes**, instead of
  showing the previous state for up to a minute.
- **A good verdict is now filled green** - audio.cpp or llama.cpp up to date, or
  a compatible TTS model present - so you can see it without reading it.
- **The selected model is its own label**: "Currently selected:
  moss-tts-local-v1.5-q8_0.gguf", rather than grey text trailing off the end of
  the line.
- The audio.cpp and TTS model labels are the same width, and the grey "last
  installed by the panel" line under the llama.cpp folder is gone - the status
  label above it says the same thing, read from llama-server itself.

## v3.76 patch75
- **Fixed: the llama.cpp update stuck on "starting..."** while the bar beneath
  it moved, and the finished row never appeared when the update completed.
- **llama.cpp now has the same status line audio.cpp has.** Under the folder it
  reads "latest version installed (b10628)", "update available - b10600 to
  b10628", or "not installed" - and where a build cannot name itself, it says
  "installed" with the commit rather than claiming to be out of date. It keeps
  itself up to date on its own, and says "checking..." until it knows.

## v3.76 patch74
- **Fixed: updating llama.cpp failed on the newest release.** llama.cpp
  publishes version releases such as v0.3.0 that contain no Windows builds at
  all - the builds are in the numbered bNNNNN releases beneath them - and the
  panel asked only for the newest release and gave up. It now looks through
  recent releases until it finds one that has the build for your CUDA version,
  preferring the numbered builds where llama.cpp keeps them.
- **The version shown and recorded is now read from the file being installed.**
  A release labelled v0.3.0 whose binaries are all b10628 is recorded as
  b10628, so the version on the page matches what you actually have.
- audio.cpp works the same way, so a release that carries no build for your
  CUDA version no longer stops the update.

## v3.76 patch73
- **The llama.cpp installer now reads your installed version properly.** It
  asks llama-server for its build number, and where a build was made without
  git history - which reports itself as version 0 - it shows the commit
  instead, or reads the version recorded in the .exe itself. It only says it
  cannot tell when none of those answer.
- **And it says "Update llama.cpp" when llama.cpp is there.** The button was
  looking at whether the panel had installed llama.cpp itself rather than
  whether llama-server.exe exists, so a copy you installed yourself always read
  as "Install".

## v3.76 patch72
- **Fixed: the llama.cpp installer reported your build as "b0".** Builds made
  without git information report their version as 0, which the panel was
  showing as though it were a build number. It now says "installed, build
  unknown", and prefers the version it recorded itself when it did the install.
- **The installer dialog is tidier.** Each CUDA build is described in a few
  words with the full detail on hover, the selected one carries a green tick,
  and only the engine name is a link - in blue - with "Install latest" as plain
  text. The paragraph about where files come from is gone; the link says it.
- **On Folder Setup, the Install/Update llama.cpp button now sits where
  "Check for update" was**, and is labelled for what it will do. The check
  button is gone, since the dialog looks up the newest release as it opens, and
  the CUDA descriptions no longer appear twice on the page.

## v3.76 patch71
- **The engine installer reads properly now.** For both llama.cpp and
  audio.cpp it shows a link to that project's releases page, your current
  version in yellow, the version available to install in green, and the two
  CUDA builds with who each is for. The highlight on the selected build follows
  the text instead of looking like a box behind it.
- **Every button on the panel is in the panel's own typeface.** Buttons do not
  inherit the page font unless told to, so they had all been in the browser's
  default face.
- The llama.cpp dialog checks for the newest release as it opens, so the
  version it offers is the one you would get.

## v3.76 patch70
- **Fixed: audio.cpp could not be installed or updated from release 0.7.0.**
  audio.cpp now publishes one build per CUDA version - 12.4 and 13.3 - instead
  of the "balanced" and "fast" builds it shipped before, so the panel was
  looking for files that no longer exist. It now picks the right build the same
  way it does for llama.cpp.
- **Both engines are installed the same way.** Pressing Install or Update for
  either llama.cpp or audio.cpp opens the same chooser: it shows the two CUDA
  builds, marks the one you already have, and reads "Update" when you pick it.
  Choosing the other build asks a second time - "Confirm CUDA 12.4 install" -
  since that replaces the build rather than updating it. llama.cpp's two
  separate buttons are now one.
- The CUDA version that was installed is remembered, so the chooser can show
  it next time.

## v3.76 patch69
- **The installation row has two buttons again**, as it should: install or
  update audio.cpp, and download the model - the second only shown while the
  model is missing. The third button, "Use what is already here", is gone
  along with the machinery behind it: the panel now sets its own folders,
  chooses a model from what is on disk and clears a stale choice by itself, so
  there is nothing left to adopt.
- **The download list is plain text rather than boxes.** Each file glows white
  under the pointer, turns accent-coloured when picked, and shows a green tick
  with "Selected for download".
- **Fixed: the Download and Cancel buttons both glowed white.** They now glow
  accent and red as intended.

## v3.76 patch68
- **A TTS with no model on disk now shows "(none selected)".** The panel was
  holding on to whichever model was selected before, and naming it beside the
  status as "selected instead", which read as though it had chosen that one.
  If a correct model does exist and you have picked a different one, your
  choice is left alone and the list still says "Incompatible model selected".
- **The download dialog is much simpler.** It names the TTS, lists the model
  files with their quantisation and size, marks the recommended one, and gives
  you Download and Cancel. You can pick more than one file - clicking a row
  again unpicks it - and the first one you choose becomes the selected model.
  The page of explanation that used to precede it is gone; where the files come
  from and their licence are one line under the list.
- Updating audio.cpp still asks before it downloads.

## v3.76 patch67
- **Fixed: the installation panel could switch to a completely different
  layout.** When the panel found audio.cpp and models on disk that its settings
  were not pointing at, it replaced the whole row with an older screen - no
  labels, no version, the claim that things were "already installed here", and
  buttons called Use it and Reinstall. That was a leftover from before the
  status lines existed, so none of the improvements since had reached it. The
  row now looks the same in every situation, reports the engine version and the
  model properly, and offers "Use what is already here" as one more button on
  it.
- **Starting TTS with no model selected now says so.** The message reads "TTS
  model - No compatible TTS model selected" rather than treating an empty
  selection as a wrong one.

## v3.76 patch66
- **You now choose which model file to download.** Instead of fetching one
  fixed file, the panel asks the model repository what it actually holds and
  shows you the list - each with its quantisation and size. The best available
  is marked "Recommended model": Q8 where there is one, otherwise Q6, Q5 or
  Q4. Where a repository offers only full-precision files, or the filenames
  say nothing about quantisation, they are simply listed as they are.
- **Fixed: Rescan could stick on "scanning".** The word was written into the
  page and only removed by a later redraw, so if that redraw did not happen it
  stayed. It now clears itself, including when a scan fails.
- **The wrong-model message gained a third option.** "Auto select model" picks
  the right model for the selected TTS by the same preference order the panel
  uses elsewhere. "Force launch wrong TTS model" is now simply "Force launch".

## v3.76 patch65
- **Every TTS now remembers its own settings.** The model you picked for it,
  both tag boards, the tag limits and whether PTI and PME are on are all
  stored per TTS. Switching away and back restores exactly what you had, so
  setting up one model no longer disturbs another.
- **A TTS that supports no tags turns PTI and PME off by itself** - there is no
  point asking a tagger for tags that will be removed before synthesis - and
  your settings are only held aside, not erased. Choosing a TTS that does
  support tags brings them back as you left them.
- **The panel picks the best model it has.** When a TTS has no model chosen
  yet, it selects the best quantisation on disk - Q8 first, then Q6, Q5, Q4,
  then any other quantisation, then a full-precision original. A model you
  chose yourself is never replaced.
- **Two clearer messages under the model list:** "No model available for
  <TTS>" in yellow when nothing on disk can serve the selected TTS, and
  "Incompatible model selected: <file>" in red when the chosen file belongs to
  a different one.

## v3.76 patch64
- **The wrong-model message now names the TTS first.** It opens with "TTS -
  Moss TTS v1.5 Local GGUF" before telling you what does not match it, so the
  choice being contradicted is stated rather than assumed.
- **A tag group with no supported tags is now just a label**, with no empty box
  drawn around it.
- **Tag boxes match the rest of the page.** The grey outline is replaced by the
  same dark glow used elsewhere on the TTS page.
- **The audio.cpp and TTS model labels** are padded like the status labels
  beside them, so each row reads as one line rather than two.

## v3.76 patch63
- **Fixed: the installation line could report on the wrong file.** With a TTS
  selected whose model you have not downloaded, the line described whichever
  model happened to be selected instead - so the dropdown said "not
  downloaded" while the line beneath discussed a completely different model.
  It now answers about the TTS you chose ("not present - 7.5 GB to download")
  and, if something else is selected, names it separately.
- **The wrong-model message says what the file actually is.** It used to read
  "it reads as audiocpp", which is a technical value every audio.cpp model
  shares and tells you nothing. It now reads "That is a Fish Audio S2 Pro
  model, but Moss TTS v1.5 Local GGUF is selected."
- **A tag group with nothing in it now says so** - "No Emotion Tag Support",
  "No Audio Tag Support" - instead of showing an empty box.
- **audio.cpp and TTS model headings** are grey labels in white text, and a
  model filename now sits clear of the verdict beside it.

## v3.76 patch62
- **Fixed: some Q8 models were reported as F32.** The panel judged a file by
  whichever tensor type held the most data, and in small models the
  full-precision parts every quantised model keeps can outweigh the quantised
  weights. It now judges by the quantised tensors, as llama.cpp does, and
  records the complete breakdown of every tensor type in the model record file
  so the reading can be checked.
- **A model the panel cannot identify is now named**: "Unrecognized TTS model:
  moss-tts-nano-q8_0.gguf" instead of a description of what it called itself.
- **The installation lines read as labels** - audio.cpp and TTS model, and
  their verdicts, are now labelled the way mandatory fields are, with any
  filename beside them shown as ordinary text.
- **Starting TTS with the wrong model selected now asks first.** If the
  selected model belongs to a different TTS, or the SenseVoice slot holds
  something that is not a recogniser, a message names the file, says what it
  actually is, points at the exact setting to change, and lists the models on
  your disk that would fit - or says plainly that none do. You can still
  **Force launch wrong TTS model**, or **Take me to setting**. The recogniser
  is only mentioned when transcripts are set to On, since the other modes load
  no model at all.

## v3.76 patch61
- **The tag boards now really are per-TTS.** Fish Audio and DramaBox were being
  offered Higgs' entire tag list on the reasoning that both accept free-form
  input. They do, but they have their own vocabularies: Fish publishes 51
  emotion markers, five tone words and a set of sounds, and says nothing about
  speed, pitch or pause - so those are gone from its board. DramaBox keeps
  pauses, speed and pitch, which its own examples demonstrate, and loses the
  expressive-range pair, which they never show. Higgs alone keeps all 43,
  because that vocabulary is its own.
- **Tags are now translated into each engine's own words.** Where Fish
  publishes a word meaning the same as ours it is used - "afraid" is sent as
  `[scared]`, "shame" as `[embarrassed]`, laughter as `[laughing]` - so
  thirteen of twenty-one emotions reach Fish as words it has published. The
  rest are sent as written and rely on its documented free-form support.
- **Fixed: the installation status could sit on "checking..." forever.** If the
  first check failed, nothing tried again until you switched pages. It is now
  a live element that animates while waiting, retries on its own, and says so
  if the panel is not answering.
- **Models without an audio.cpp family tag are now identified too.** A file
  like an LTX quant sitting in the models folder used to show uncoloured; it
  now reads as what its header declares and is coloured accordingly. Only a
  file that declares nothing at all stays uncoloured.
- **The TTS record file now covers everything the TTS pages look at**, not only
  audio.cpp models - so any file shown in a model list has an entry explaining
  how it was identified.

## v3.76 patch60
- **Fixed: a freshly opened TTS page claimed nothing was installed.** Until the
  first check came back, the page reported no audio.cpp and no model - which
  went away if you switched to another page and came back. It now says
  "checking..." until it has an answer.
- **Fixed: one model could be mistaken for another.** MOSS-TTS-Nano was being
  recognised as MOSS-TTS-Local, because family names were matched loosely. A
  model that states its family is now matched exactly, so no file is taken for
  a different model. Checked against all 37 model families found on a real
  installation.
- **Safetensors model folders are now identified too.** They have no header to
  read, but audio.cpp's own packages include a `config.json` that usually names
  the family - so a Higgs folder is recognised as Higgs and coloured like any
  other model. Only a folder whose config names nothing stays uncoloured, and
  hovering it says which of the two it was.
- **The TTS record file now says what the panel concluded**, not just what each
  file declared: every entry carries the family the panel settled on and what
  that means in words - "tts pack: Higgs Audio v3 (4B)", "recogniser", "another
  audio.cpp family - the panel carries no pack for it", or "not identified".
  The record files rebuild once on this update.

## v3.76 patch59
- **Fixed: servers would not launch at all.** The Full-size SWA cache setting
  added in patch51 was being written into the launcher as `--swa-full off`,
  but llama.cpp takes that switch with no value at all - so llama-server
  rejected the argument and exited immediately, with "error: invalid argument:
  off" and no server on any slot. The switch is now written the way llama.cpp
  expects: absent when off, and on its own when on. Nothing else about the
  setting changed.
- **SWA Meta is now shown as a label** beside Built-in MTP head under the model
  picker, so the two facts a model file carries read as a matched pair.

## v3.76 patch58
- **TTS models now have their own record file.** `tts-model-kinds.json` sits
  beside `model-kinds.json` and holds everything the panel has read about your
  audio models - which family each file declares, how it is quantised, and the
  reason the TTS pages read it the way they do - laid out one record per line
  like the language-model file. Both files rebuild themselves once on this
  update, so the first scan after installing will take a moment.
- **Fixed: the TTS dropdown and the installation line could disagree.** The
  dropdown said "not downloaded" while the line below said "present", because
  the dropdown only looked in the folder an install would have used while the
  line searched everywhere. Both search everywhere now, so a model kept in your
  own folder layout counts properly.
- **Safetensors folders are no longer left uncoloured.** They have no header to
  read a family from, which is a fact about them rather than a failure - so in
  the SenseVoice list, which needs a `.gguf`, they are red, and in the model
  list they stay plain with a note saying why.
- Removed the "1 click install into the PandorumLLM directory" note - the
  button beside it says the same thing.

## v3.76 patch57
- **The model lists now colour every entry.** Models that belong to an engine
  the panel does not carry - VibeVoice, VoxCPM2, Confucius4 and so on - were
  left white, as though the panel had failed to read them. They had in fact
  said exactly what they were, so they are now red like any other model that is
  not the one you selected. White is reserved for a file that says nothing
  about itself at all.
- **The SenseVoice list stops hiding speech models** and shows them in red
  instead, so you can see at a glance that a TTS model is the wrong choice
  there rather than wondering why the list looks short.
- **Every model read is now recorded in `model-kinds.json`, with a reason.**
  TTS models are listed beside the language models, each with a line saying how
  the panel identified it - "declares audio.cpp family higgs_audio_tts", or
  "architecture audiocpp with no family key". If a model of yours shows white,
  that file now says why, and it is a useful thing to send along with a report.
- **`model-kinds.json` is readable.** It was a single enormous line; it is now
  one entry per line, sorted.
- **Tidier TTS page.** The audio.cpp releases address and its "Check for
  update" button are gone - the panel checks the version by itself and reports
  it on the Installation line. "Repaired Sample Vault" is now simply "Sample
  Vault" and, like the other folders, is one PandorumLLM creates and owns.

## v3.76 patch56
- **Fixed: the panel could not recognise any audio.cpp model.** Every model
  file audio.cpp produces describes its architecture as "audiocpp" and names
  the model family it really is in a separate field. The panel was reading the
  first one, so every file looked the same - which is why the installation
  panel reported "present, but it calls itself audiocpp" for a model that was
  sitting in the folder, and why the model list showed no colours. It now
  reads the field that names the family, so Higgs is Higgs, SenseVoice is a
  recogniser, and a model for another engine is named as such.
- **The model list colours work again as a result:** green for a file that is
  the TTS you selected, red for one that is a different model or a recogniser,
  plain for a family this panel does not know.
- **The TTS folders are now the panel's own.** PandorumLLM creates and keeps
  `audio.cpp`, `Models\TTS`, `TTSaudio` and `Sample Vault` beside itself.
  They are still shown on the TTS page so you can see where your files live,
  but they are read-only now - a wrong or stale folder was behind every case
  of the panel failing to find models it had found before.

## v3.76 patch55
- **Correction: Chatterbox is sent no tags.** Patch54 gave it `[laugh]` and
  `[cough]`, which Resemble document for their Turbo and Nano models. But
  audio.cpp - which is what actually runs the model here - states that its
  Chatterbox integration offers voice cloning and voice conversion and no tag
  interface at all, so those brackets would have been read out as words.
  Chatterbox now receives clean text.
- **Higgs' Top K default is now 30, was 50.** audio.cpp ships 30 for this
  model and explains why: it is less prone to ending a clip early than the 50
  used by the original Python client. Ending clips early is a fault some
  setups see on short lines, so the engine's own number is the better default.
  This only applies to new installs and when you switch TTS - if you already
  have 50 set, it stays until you change it, and you can try 30 by hand on the
  TTS page.
- **Fish Audio's samplers are left to the engine.** The 0.7 temperature came
  from Fish's own Python library, which is not what plays your audio;
  audio.cpp has its own tested defaults for this model, so the panel now sends
  neither value and lets them stand.

## v3.76 patch54
- **Each TTS now gets the tags it can actually perform, written the way it
  reads them.** Until now every model was handed Higgs' tag system or nothing.
  Researched per model and implemented per model:
  - **Higgs Audio v3** - its own control tokens, as before.
  - **Fish Audio S2 Pro** - takes free-form bracket words, so the whole
    vocabulary now reaches it: `[angry]`, `[laugh]`, `[whispering]`. It was
    previously having every tag stripped.
  - **DramaBox** - not a tag system at all. Its lines are now rewritten into
    the format it documents: speech inside double quotes, performance
    directions outside them, using the character's own name - `Lydia sounds
    angry. "Out of my way." Lydia laughs. "Fool."` Quotes already in the
    dialogue become single quotes so they cannot close the speech early.
  - **OmniVoice** and **Chatterbox** - only the tags they publish, `[laughter]`
    and `[sigh]` for one, `[laugh]` and `[cough]` for the other. Nothing is
    invented around them, because an unrecognised bracket is a word the model
    reads out loud.
  - **MOSS-TTS-Local** keeps its pause marker; **DotTTS-MF** and
    **PocketTTS-ENG** perform no text tags, so none are sent.
- **The tag boards now populate from the selected TTS and swap when you change
  it.** You only see switches for tags that engine performs; where it performs
  none, the board says so instead of showing controls that do nothing.
- **The PTI and PME prompts switch with the TTS too.** The tagger is asked for
  exactly what the chosen model can perform - the full vocabulary, a handful of
  words, or no tags at all, which also makes the prompt considerably shorter.
- **Fixed a subtle one:** the onomatopoeia the panel injects for Higgs (a bare
  Higgs tag performs nothing) is no longer sent to models that perform the tag
  themselves - they would have laughed and then said "Hehe".

## v3.76 patch53
- **Fixed: a TTS model was looked for in one place only.** If the file an
  install would have written was missing or another model sat where it was
  expected, the panel could end up reading the wrong file entirely - a
  SenseVoice recogniser in the Higgs folder produced "present, but it calls
  itself audiocpp". The panel now searches the whole models folder and
  identifies every file by its own header, so the right model is found
  wherever you keep it, in any subfolder, under any name. A model you have
  selected yourself is still preferred whenever it fits.
- **Both model pickers now colour their entries.** In the TTS model list, a
  file whose header says it is the selected TTS shows green; one that says it
  is a different TTS, or a speech-to-text model, shows red. The SenseVoice
  list does the same in reverse. A file that declares nothing recognisable is
  left plain rather than marked wrong - unreadable is not the same as
  incorrect. Hover any entry to see what it declared.
- **The SenseVoice list no longer hides models by name.** It used a fixed list
  of words, which knew nothing about the six TTS models added in patch49 and
  would have offered them all as recognisers. It now hides what is actually a
  voice model and shows what is actually a recogniser.

## v3.76 patch52
- **Tags read as stage directions in the dashboard.** The tag record used to
  show raw markup - `[EMOTION-AMUSEMENT] You call that a sword? [LAUGHTER]`.
  It now reads `*amused* You call that a sword? *laughs*`. Every form is
  covered: the engine's own tokens, the bracket form the model has to write
  because SkyrimNet strips angle brackets, and SkyrimNet's bare tags like
  `[chuckles]`. A bracket that is not a tag - `he drew [the sword]` - is left
  exactly as written.
- **Every TTS is now checked for tag handling, not just every kind of TTS.**
  The test suite walks the whole model table and asks each one the same
  question: is every tag either turned into something that engine performs, or
  removed? Nothing is ever handed over as plain text, because plain text gets
  read aloud. Any TTS added in future is checked the day it is added.

## v3.76 patch51
- **Sliding-window models are recognised.** If a model was trained with
  sliding-window attention, the panel now reads the window out of its header
  and shows **SWA Meta: 4096** (or whatever it is) next to the Built-in MTP
  head label under the model picker. It is a property of the model, not a
  setting - llama.cpp only applies a window on architectures built for one.
- **The allocation report shows the second cache.** llama.cpp gives a windowed
  model two KV caches - a full one for its ordinary layers and a small one
  sized to the window - and the report now lists that second one as its own
  **SWA cache** row, with where it landed. This is why such models are so
  cheap at long context.
- **Colours on the allocation report.** VRAM is green, CPU is yellow, system
  memory is plain - and turns red only when weights actually spilled into it.
  A spill also prints **RAM Offload Detected!** under the report, so the case
  that costs you speed is impossible to miss, while the pinned buffer every
  healthy launch has is left alone.
- **Fixed: mmapped weights were counted as VRAM.** Weights held in system
  memory under llama.cpp's `CPU_Mapped` buffer were added to the VRAM total,
  so a model that half-fitted looked like it fitted. They now count as host
  memory and raise the spill warning.
- **New setting: Full-size SWA cache** (`--swa-full`), off by default. It
  makes the windowed layers' cache full-size: more VRAM, but prompt-cache
  reuse and context shifting work again. The report says when the server is
  re-processing whole prompts, which is the symptom this fixes.

## v3.76 patch50
- **The TTS page now tells you what you actually have.** The single
  "Installed" badge is gone. In its place are two lines: **audio.cpp**, which
  says whether the engine is missing, is not audio.cpp at all (pointing a
  folder at llama.cpp is a real mistake people make), is too old for these
  models, has an update waiting, or is current; and **TTS model**, which says
  whether the selected model's file is actually there. Each line has its own
  button, so the engine is installed once and models are downloaded per model.
- **The model check reads the file, not its name.** The panel opens the GGUF
  header and reads the architecture the file declares and the quantisation its
  tensors are stored in - so a Higgs model renamed into the DramaBox folder is
  caught and named, rather than counted as DramaBox. A present model reads,
  for example, "present - DramaBox, Q8_0".
- **audio.cpp is no longer re-downloaded for every TTS.** Installing a TTS
  used to reinstall the engine every time, which made sense with one model and
  makes none with eight. Now the engine is fetched only when the check says it
  must be - missing, wrong, too old, or out of date - and left alone when it is
  already current. audio.cpp 0.6 or newer is required, since that is the
  release where the GGUF Q8 support these models need arrived.
- **The dropdown shows the backend and what is missing.** Each entry now reads
  like "DramaBox [Audio.cpp]", and any model whose file has not been
  downloaded is listed in yellow with its download size.

## v3.76 patch49
- **Six more one-click TTS models.** The dropdown at the top of the TTS page
  now offers Moss TTS v1.5 Local GGUF (7.5 GB), DramaBox (18.9 GB), DotTTS-MF
  (3.0 GB), OmniVoice (1.4 GB), Chatterbox (2.1 GB) and PocketTTS-ENG
  (128 MB), alongside Higgs and Fish. Pick one and press Install: the panel
  fetches audio.cpp and that model's Q8 file, then points every field at them.
  Each option shows its download size before you click, and the confirm dialog
  names the model's licence.
- **Chatterbox is voice-clone only** - it has no plain speech mode, so every
  line needs a voice sample, which is how the panel drives TTS anyway. The
  dropdown says so.
- **Tags now follow the model, not the engine.** Higgs reads the panel's
  emotion and prosody tokens; MOSS-TTS-Local keeps its own pause marker
  whether it runs on the MOSS server or on audio.cpp; the rest read neither,
  so tags are removed rather than sent - a model reads an unknown token out
  loud instead of obeying it. Sampler defaults follow the same rule: each
  model gets the values its own documentation publishes, and where none are
  published the fields stay empty so the engine's own defaults stand.

## v3.76 patch48
- **llama.cpp can update itself, from Folder Setup.** Beside the llama.cpp
  folder there are now two update buttons, one per Windows CUDA line. Either
  one fetches the newest release from GitHub - the binaries *and* the matching
  CUDA runtime DLLs, which llama.cpp ships as a separate archive - and unpacks
  both into your llama.cpp folder. Nothing in that folder is deleted: the
  archives overwrite the files they own and leave your launchers and scripts
  alone. Stop any running server first, and restart it afterwards.
- **Two lines, because they are not interchangeable.** CUDA 13.3 is for RTX
  20-series and newer (driver 580+). CUDA 12.4 is for GTX 10-series, Titan V
  and older, which CUDA 13 no longer supports at all - and it cannot drive an
  RTX 50-series card, which needs the 13.3 build. Each button says who it is
  for, and the panel remembers which build and line it last installed.

## v3.76 patch47
- **Fish Audio S2 Pro is now a one-click install, alongside Higgs.** Pick it
  in the TTS dropdown at the top of the page and press Install: the panel
  fetches the latest audio.cpp and the Q8 model (about 6.3 GB) into its own
  folder, then points every field at them, exactly as it does for Higgs. The
  confirm dialog names the model, its size and its licence before anything is
  downloaded - Fish ships under the Fish Audio Research License, which is free
  for research and non-commercial use.
- **Switching TTS selects the right model for you.** Choosing Higgs or Fish at
  the top now also sets the matching Q8 model file, its request id, and that
  model's own reference samplers - Boson's temperature 0.8 with top_k 50 for
  Higgs, Fish Audio's published temperature 0.7 for Fish, whose sampling has
  no top_k at all, so that field is left empty and the engine's own default
  stands.
- **Tags on Fish.** Fish understands speaker-turn markers and nothing like
  Higgs' emotion and prosody tokens, and a model reads an unknown token out
  loud instead of obeying it. With Fish selected the panel therefore removes
  tags rather than sending them, the same way it already does for MOSS - so
  lines come out clean, and no tag is ever spoken.

## v3.76 patch46
- **The Launch / Stop / Terminal buttons are back.** Patch45 gave the control
  row its own element so it would stop flashing, but the server cards never
  handed it its place on the card - so after the first redraw the row came up
  empty and the buttons were simply gone. The card renderer now puts both the
  control row and the allocation report back in place itself, every time it
  draws, and each of them takes its place whenever one is free. The buttons
  show the correct state again, and still never flash.

## v3.76 patch45
- **The launch controls no longer flash either.** The report held still after
  patch44, but the button row above it kept flashing: the old stuck-button
  cure forced the whole card to rebuild on every state change so the button
  could never latch - and that rebuild was the flash. The Launch / Stop /
  Terminal row is now its own persistent element, exactly like the report:
  built once per server, carried across every card repaint as the same node,
  and patched in place, with a soft transition on state changes. The
  stuck-button protection is unchanged - a failed or hung launch still frees
  the button on its own - it just no longer costs a redraw.

## v3.76 patch44
- **The allocation report no longer flashes - at all.** Two remaining causes
  found and closed. The first poll after a launch press could race ahead of
  the watermark and read the previous log once; the poll now starts only
  after the launch call returns. And every card repaint (launching, running,
  stopping) rebuilt the report's HTML wholesale - the report is now its own
  persistent element: created once per server, carried across every card
  rebuild as the same node, and updated row by row in place. A value that
  lands pulses briefly in the accent and settles. Nothing else on the page
  can blink it.

## v3.76 patch43
- **The allocation report no longer flashes the previous launch.** The
  launcher writes a new numbered log per launch, so the first polls after a
  press were reading the old one: full sizes appeared instantly, vanished when
  the new file arrived, and returned as it filled. A launch now stamps a
  watermark, and only a session dated at or after the press may speak - until
  it exists the card shows the clean queued/allocating lifecycle, from the
  instant of the press.
- **Three allocations the newer engine builds report differently are now
  counted.** The vision projector announces itself through clip_model_loader
  and reports its weights as "model size" (1.1 GiB on the reporting rig) - now
  a Projector row. Its graph reserves through reserve_compute_meta, including
  a plain CPU buffer - now inside Compute and Host. And the drafter's caches
  print only summary lines - now a Draft KV row, counted once (the main model
  prints both forms; summaries that echo a per-device line are skipped).
- **"Host spill" is now "Host buffers", and every row explains itself.** Hover
  any label: Host buffers are the CUDA_Host / CPU allocations the engine
  itself reports - pinned system RAM staging and scratch, never VRAM. The MTP
  drafter's memory lives in the Draft rows on builds that no longer print a
  separate estimate.

## v3.76 patch42
- **Meta's pick now has to stand in the scene.** The next-speaker guess
  sometimes named the wrong target or pasted its own prompt example back. A
  pick must now match a character who has actually spoken inside the current
  pairing window - knowing the name from an hour ago is no longer enough. A
  refused pick doesn't warm a voice and doesn't influence speaker pairing; the
  TTS terminal says "Meta named X - not in this scene; the strip rule stands",
  and the first-chunk tag strip covers that switch exactly as if no guess had
  been made.
- **The realtime factor audited: it is generation alone.** The x-realtime
  number divides audio length by the winning synthesis attempt only. Player
  tagging, mood reading, token estimates, thought audio, holds, and discarded
  retry attempts are all measured and named separately (the prep line and the
  server's own generate split) - none of them ever sit inside the factor. No
  behaviour changed; the test suite now pins every boundary.

## v3.76 patch41
- **The tag principle is now guarded, not just true.** Everything the Allowed
  Tags and Player Audio Tags boards enable is offered to the player tagger and
  the mood reader, and can reach the speech engine; click a word off and it
  leaves both prompts - examples included - the moment you do. Nothing is ever
  removed by a hidden list again: a removal is a default-off switch you can
  turn back on. The panel's own tags keep working whatever the Audio Tags
  trust switch says, which governs SkyrimNet-written tags alone. (No behaviour
  changed here - the test suite now proves all of it on every run.)
- **The TTS page's paragraph-long titles are gone.** Settings like Voice
  Prime, Runaway Handling, Chunk Size and Kept Takes now show a short title
  with the same little ? chip the rest of the page uses - hover it for the
  full explanation. Nothing was reworded away; every sentence moved behind
  its chip.

## v3.76 patch40
- **The small launch terminal on server cards is gone.** It scrolled far too
  fast to read during a load, so it told you nothing. Its job passes to the
  allocation report below.
- **The VRAM report grew up.** Instead of one crowded line it is now a stacked
  list - one row per allocation llama.cpp names for the instance: Weights,
  Projector, Draft weights, KV cache, Draft KV, Recurrent state, Compute, MTP
  context, with a Total, host-RAM spill, and free-VRAM-at-load underneath.
  During a launch each of the three ever-present rows shows where the loader
  is: "queued" until the loader reaches it, "allocating..." while it is the
  next size owed, then the size itself. A load that dies keeps the numbers
  that landed and shows a dash for what never came - which is usually the
  diagnosis in one glance. Every sized row also carries a chip saying where
  the allocation landed - VRAM (GPUs named when an allocation splits across
  several) or RAM for host-side buffers - and if the log shows an allocation
  failure, the row the loader owed is marked OOM in red. The attempt list
  persists after the launch ends, and the report greets you with the last
  launch's numbers when you open the page.

## v3.76 patch39
- **The provider drop boxes explain themselves.** New users kept reading the
  empty box on the Live Network as a one-provider slot. It now says "drop any
  number of providers here to link to server" - in the panel accent with a
  soft glow, centred in the box - and disappears the moment the first provider
  lands. The box itself hasn't changed size or behaviour.

## v3.76 patch38
- **Broken voice-clip headers can be repaired from the panel.** Some voicetype
  WAVs come from tools that never finish their own header - the file claims
  more audio than it holds (often a `data` size of 0xFFFFFFFF) and the speech
  engine refuses it with `failed to read WAV data chunk`. Two new buttons under
  Local Voice Clips: **Scan Voice Folder** counts the damage, **Repair
  Headers** fixes it in place. Every repaired file is copied into
  `_header-backup` first, and only the two size fields are rewritten - the
  audio bytes are untouched. Honest files are left alone.

## v3.76 patch37
- **The thought queue is rolled back.** Patch34 held the next line's generation
  until the previous thought finished playing, and in real sessions every turn
  paid for that wait. The old timing engine is back: a thought follows its own
  spoken line, and only the TTS delivery of the NEXT line waits on it - capped,
  and never for the player or the startup ping. The Delay field stays (default
  0.45 s) and feeds the engine directly.
- **Remote access can open provider job payloads.** Clicking a finished job in
  the dashboard works from a remote session now - the prompt and the reply open
  the same way they do on the panel PC.
- **Audio Cache auditions can cross to the other PC.** A new Audition dropdown
  picks where a clicked take or sample sounds: In panel or In remote. Clicking
  from the other side answers "sent" and the chosen side plays it.
- **The warm-up chunk shows itself.** The hidden voice-switch chunk logs as
  "Dummy Chunk" with a gold halo in the TTS terminal, and the tag-strip
  fallback line glows white - one glance answers whether the primer fired.

## v3.76 patch36
- **One name, one voice.** The identity ledger (797 field blocks) shows every
  line leaving through the sample its request asked for - and three NPC lines
  NAMED after the player on the way, teaching the cache `Maxxor ->
  maleyoungeager`. The patch40 rule guarded only the queue rung; the pin, the
  run and the Meta pick walked past it. `player_name_misfit` is now the one
  spelling, asked at every rung: the pin refuses at the origin (alarmed), a
  polluted run is dropped before it is obeyed (self-heal), the Meta pick
  filters its candidates (gotcha 200). Also: `slot_status` trusts a down/
  wedged verdict for 3.0 s (`_ST_TTL_DOWN`) - the owner's box paid 2 x 0.35 s
  on every state read for two silent ports - and any Launch/Stop click clears
  the whole probe cache so the next read asks the world. Wrong-voice as HEARD
  is the unprimed-drift mode p35 fixed; `max_tokens before EOC` did not
  reproduce anywhere in a week of logs.

---

## v3.76 patch35
- **The cache answers.** Three field reports, one popup. The patch31 primer
  never ran once: `tts_voice_prime_kick` declines a held `_spk_lock`, and
  `meta_note` - its only caller - held that lock at the call site; the kick
  now fires after release (gotcha 199). Audition playback is fetch-first: a
  refused request paints the row with the server's own words and every
  refusal reaches the panel log, instead of dying in an `onerror` that said
  nothing. The popup sizes to content (`width:max-content`, viewport-capped);
  the sample filename plays the learned sample through `tts_sample_path_for`
  - a door that resolves by name alone and never accepts a path from the
  page; and the takes-view listener binds to its own fresh table, ending the
  stacked-closure accumulation on the overlay.

---

## v3.76 patch34
- **The turn queue (SS14).** Thought audio is `after`-only: one plain timer
  at the bundle's close (`tts_chunk_is_last`) voices the freshest thought,
  the last chunk's own duration plus a late-biased delay later; the page's
  `au.onended` report releases the turn; and the NEXT Dialogue completion is
  held at the proxy (`th_turn_wait`) until then, ceiling-bounded so a closed
  page loses nothing. Deleted with SS14.4: `TH_AFTER` and its re-arming
  timers, the defer ladder, the three timing allowances, the predicted
  floor, `_hold_s` and the TTS-door hold, and the `ttsThoughtSeq` branch -
  plus the write-only inflight registry the deletion orphaned. The Sequence
  select becomes the Delay field (`ttsThoughtDelayS`, ships 0.75, biased
  late because late is now the safe side). Ships together with patch33 in
  one delivery.

---

## v3.76 patch33
- **The blade finds the tokens.** Patch31's strip ran ninety lines before
  `tts_apply_tags` - cutting `<|...|>` tokens at a point where none existed,
  a no-op in the field: the function was proven, its pipeline position never
  was (gotcha 198). The ladder still decides early, before this line's own
  synthesis moves the conditioning mirror; the cut now sits after token
  birth, on `processed`, with a three-index order pin so it cannot drift
  back. And `TTS_ALIAS` completes the engine's emotion family - every value
  in `TTS_TAGS["emotion"]` has typed words, coverage gate-scanned with a
  synthetic control; `[content]` deliberately excluded as ordinary dialogue.

---

## v3.76 patch32
- **The Audio Cache keeps house.** `ttsTakesKeep` (default 50, both engine
  sheets, 0 = unlimited): as each new line saves, the oldest takes past the
  cap fall - scanned by the exact-timestamp rule, never a glob, so
  `Urag_gro-Shub`'s history survives pruning `Urag` (patch8's law), and each
  removal is said once in the TTS terminal (`take cap` joins the key
  vocabulary). Every kept take in the popup is click-to-audition through one
  validated door (`/api/tts-audio?take=`): basename, exact prefix, exact
  stamp, or nothing - and an audition never posts `tts-played`, which is the
  turn queue's accounting. The created column (on the page since patch8) is
  now pinned. The controls run taught the glob-nin to catch aliases.

---

## v3.76 patch31
- **Voice Prime.** On a speaker switch, the Meta selection's speaker name is
  looked up in the learned voice pairings and the incoming sample is warmed
  with one hidden `tts_acpp_speak` while the Dialogue model still generates -
  a minimal sibling of the thought path (`TTSW.blk`, the conditioning mirror
  told, nothing else touched: no speaker pin, no tagger, no spoken record,
  no fit stats, no saved file). At the speak path, a three-rung ladder that
  never holds: no switch or a warmed switch releases untouched; an unprimed
  switch drops the `<|...|>` tags that start inside the first chunk window
  (`ttsChunkChars`) - a tag priced below latency, per the owner. Both events
  land in the TTS terminal (`prime`, `tag strip` join the key vocabulary);
  the Calibration ledger mirror is deliberately deferred until its renderer
  is next opened. `ttsVoicePrime` on the TTS page, default on; off is exact
  passthrough. Grounded in the v3.75 A/B evidence: plain-text switches are
  clean, the one proven field failure was a tag-fronted first take.

---

## v3.76 patch30
- **The record of the system, docs only.** DEVELOPMENT.md section 15 is the
  design record of the unified presentation system - tables, resolvers,
  reporter, differs, the corpus law, and what is deliberately NOT unified -
  pinned by the gate so it cannot silently vanish. Two gate narratives
  retold to the present; the monolith's size told to the current build.

---

## v3.76 patch29
- **A counted figure wears no tilde.** The Proxy record line writes `(412)`
  for a server-counted reasoning figure and keeps `(~398)` only for an
  estimate - the owner's ruling, and the pair `think_tokens` has answered
  since patch13; the Thinking header already wrote it honestly, the record
  line catches up. `proxy_line_text` gains `tn_exact` with estimate as the
  default, so no caller can forget its way into a false exactness; the
  short-form reader takes both spellings. The p16 corpus carries estimates
  only, so all five feeds stay byte-identical and the writer is proven by
  direct call, both ways, in the gate.

---

## v3.76 patch28
- **One entity, one ink, one asker - no visible change.** `entInk` is the
  entity resolver's public face: drilldown-strip, ready-set hit, derivation
  fallback (the hash stays live - an id standing in for a title, a provider
  mid-arrival, the IntelEngine case). `dashColor` is internal, pinned at
  exactly three references: its definition, the routing-build fill, the
  fallback. The five entity surfaces (chart, two network cards, the hover
  link, the chips) ask `entInk`; `paintThink` takes the ink OFF the record -
  resolved once per routing change, not once per painted token. New
  `gate-tools/entity-diff.js`, shipped with the gate section that runs it:
  a snapshot of the ready set plus probe answers (known, drilldown, unknown,
  empty), diffed byte for byte between builds; an old build answers through
  the derivation itself, so the p27-vs-p28 run IS the consolidation proof -
  identical. Self-vs-self identical; one palette byte moved reads DIFFERENT.
  All five feeds byte-identical.

---

## v3.76 patch27
- **The spoken family reads the declarations, no visible change.**
  `paintSpoken`, `paintTtsMeta` and `rtTint` are pane-threaded; every ink
  arrives through `inkOf(pane, element)` with the fault seam, and a PARTS
  declaration answers a scalar reader with its named part - one row serves
  the Proxy arm's decomposition and a TTS-family fragment on the same pane.
  `said`/`think`/`rt`/`unit`/`file`/`key`/`tag`/`name` are live; the
  dashboard's spliced-family inks are now DECLARED (patch22/23's sameness,
  written down). New single sources: the tag shape (bracket classes - the
  gate's own `[]]` ban caught the first spelling, whose pause branch could
  never have matched), the unit-word list, the `.wav` rule, the key
  vocabulary, the tts number form; the meta stamp twin folded to the derived
  strict rule (437/437 corpus-equal) and `TSTAMP_RX` derives from
  `ELEM.stamp.src` - two named stamp spellings remain in the page, pinned at
  exactly two. One declared delta, owner-reversible in one row: a bare
  number in a dashboard-painted fragment reads the dashboard's green value
  part rather than meta gold; no such line exists in any corpus.
  Narrowings, corpus-clean: `TSTAMP_RX` takes a space, not any whitespace;
  the meta stamp accepts the strict form only. Proven byte-identical on all
  five feeds.

---

## v3.76 patch26
- **The think family reads the tables, no visible change.** `drawSegs` is now
  pane-aware: it consults `SHOW[pane]` as the single authority - a hex is the
  ink, `ENTITY` takes it from the record, `null` marks nothing, and an element
  the pane never decided draws as a dotted fault with a reference, never a
  quiet default. There is deliberately no legacy path: a caller that forgets
  its pane crashes the render loudly. `paintThink`'s private rule ladder is
  gone - `sepRx` and the INPUT/OUTPUT test moved into `LINE` (with the trim
  they always applied), `timeRx` became `ELEM.stamp`'s named `loose` form
  (verbatim; all 322 corpus stamps are strict-equal, so the tolerance is
  documented rather than exercised), and `portRx`/`tokEstRx` fell to the
  RXTOK derivations - the header figure now has ONE source in the file. The
  `#8b93a3` port literal is gone. Proven byte-identical on all five feeds
  against the p16 session logs.

---

## v3.76 patch25
- **The unified presentation core, no visible change.** One table (`ELEM`) now
  holds every element's recognition rule beside its base ink; a second (`SHOW`)
  declares, exhaustively, what each of the five terminals does with each
  element; a third (`LINE`) holds the whole-line shapes in their load-bearing
  order. A dual-vector reporter audits the tables at page load and reports any
  gap from both sides - undecided in a terminal, or declared but undefined -
  through `/api/paint-fault` onto the panel's existing warn/error split, and a
  fault that reaches the screen is drawn dotted and colourless rather than
  borrowing a deliberate ink. The Proxy terminal is the first painter migrated
  onto the tables - proven byte-identical against a real session's logs on all
  five feeds with the patch24 differ - and its private rule ladder (the file's
  fifth port regex among them) is gone. `MARK.rx` is now derived from the same
  sources. Approved for patch29: a counted token figure will drop the `~`.

---

## v3.76 patch24
- **Internal tooling and documentation, no visible change.** The whole-terminal
  differ that proves a painter migration moved nothing is now executed by the
  build gate itself: painting a feed against an identical build must report
  identical, and a build with one colour deliberately moved must be caught and
  fail the run. It shipped in patch23 pointing at a path that existed only on
  the machine that wrote it, with no exit code - present, encoding-checked, and
  unable to say no. It also gained a whole-feed mode, so the spliced spoken
  lines and the tree corrections are compared too, and every `gate-tools`
  require is pinned relative. The developer notes were corrected to match the
  tree: 17 shipped files, the real line count, five gate tools, one build
  command instead of two.

---

## v3.76 patch23
- **The realtime reading on a spoken line shown in the Proxy terminal is coloured
  like it is everywhere else.** With Dialogue Text switched on, the `4.42x` at the
  end of a spliced line was plain grey while the same figure was gold in the TTS
  terminal.

---

## v3.76 patch22
- **Turning timestamps on in the TTS terminal no longer breaks spoken lines.** With
  them shown, the speaker's name lost its colour and the mood icon lost the fixed
  cell that keeps names lined up under each other. Both are back, and the timestamp
  itself now matches the colour every other terminal gives it.

---

## v3.76 patch21
- **Timestamps look the same in every terminal.** The Proxy terminal used a slightly
  different blue from the other four.
- Spoken lines and thoughts now take their colours from the same place every terminal
  reads, so a change lands everywhere at once. Nothing else moved: each terminal keeps
  its own colouring for numbers, durations and anything it alone shows.

---

## v3.76 patch20
- **A thought no longer holds the next line back longer than it needs to.** The panel
  was estimating how long a thought would take to play, from the length of the file,
  and waiting out that estimate. The page now reports the moment it actually finishes,
  so the wait ends when the thought does. If a page is closed or asleep and never
  reports, the old estimate still applies, exactly as before.

---

## v3.76 patch19
- **Internal tidying, no visible change.** The TTS terminal kept its own copy of
  three colours the rest of the panel already shared. It now reads the shared set,
  so a colour changes in one place or not at all. Every line the terminal drew
  before it draws identically after - checked against 721 lines of real log.

---

## v3.76 patch18
- **Timings no longer count time the machine spent asleep.** Durations the panel
  measures - how long a request took, how long a model generated, how long speech
  took to produce - were read from the system clock, so a sleep or a clock
  adjustment in the middle of one was counted as part of it. One log showed a status
  read taking six and three quarter hours. They now use a clock that only moves
  forward.

---

## v3.76 patch17
- **Clicking a provider in the Proxy terminal opens its request and reply again.**
  It had started jumping to Live Network instead, after the provider list finished
  loading. The name is the payload link there, as it always was.
- **Headings in the Thinking and PTI/PME terminals show the provider as the link**,
  rather than a tag reading "Request". They only did that when timestamps were
  shown, which is off by default in those terminals.
- **That tag also errored when clicked.** It now opens the request and reply the
  same way every other link in the panel does.
- **The player's voice and the player's thoughts can share one recording.** They
  are the same person, but the panel had been treating the second one to arrive as
  an impostor and refusing the line.

---

## v3.76 patch16
- **A provider looks the same in every terminal.** Its name only ever carried its
  colour and emoji in the Proxy terminal; in the Thinking, PTI/PME and Calibration
  feeds it was plain grey text. Every terminal now draws it from the same list -
  same colour, same emoji, and the same hover glow, wherever the name appears.
- **Clicking a provider name anywhere opens it in Live Network.**
- **A provider whose name is part of a longer word is left alone**, so a provider
  called Vision does not light up inside "Visionary".

---

## v3.76 patch15
- **A spoken line links to the call its words came from.** Every line in the TTS
  terminal now carries a Request tag - clicking it opens the request and reply that
  produced those words. Lines too short to belong to one reply, and lines whose
  words appear in two, carry no tag rather than a wrong one.
- **The replay marker is untouched**, and the copy of a spoken line shown under its
  completion in the Proxy terminal carries no second link, because the completion's
  own line already opens that request.

---

## v3.76 patch14
- **Completed reasoning now shows which provider produced it, as a link.** Each
  heading in the Thinking terminal carried a tag reading "Dialogue" next to the
  provider's name, whatever the provider actually was. The provider's own name is
  now the link, and clicking it opens that call's request and reply - the same as
  clicking the provider on the call's line in the Proxy terminal.
- **The same in the PTI/PME terminal**, where the heading names the job.
- **One link per line.** An action branch carried a second link to the request its
  own record line already opens, one row above it. It doesn't any more.

---

## v3.76 patch13
- **The reasoning token count agrees with itself.** The figure on a call's record,
  on its heading in the Thinking terminal and in its payload window came from three
  separate calculations, and two of them could differ from the third by one.
- **It uses the real count when llama.cpp reports one.** The record and the heading
  always estimated from the length of the text, even on calls where the server had
  counted the tokens exactly. A figure that was counted now reads `(412 tok)`; one
  the panel measured still reads `(~398 tok est)`.

---

## v3.76 patch12
- **The bad-match warning on draft models and vision projectors works again.** The
  panel remembers what it read out of each model's header, and two releases ago it
  started reading one more thing - the width a file speaks at - without telling that
  memory. Every model listed before then kept an entry with no width in it, so the
  check had nothing to compare and stayed quiet. Those entries are re-read on the
  next start, and a mismatched drafter or projector is marked again.
- **The first start after this update is slower.** The panel re-reads the header of
  every model whose remembered entry is incomplete. It happens once.

---

## v3.76 patch11
- **The Thinking link in the Proxy terminal works.** A call that produced reasoning
  was printing its link marker as raw text at the end of the line instead of showing
  a Thinking tag. The tag is now drawn, and clicking it scrolls the Thinking terminal
  to that call's reasoning and flashes it.
- **An action links back to the request that chose it.** The action a character
  picked now carries a Dialogue tag, so the branch opens the request and reply it
  came from - the same link the player tags and mood records already had.
- **A link lands in the terminal it names.** The Thinking, PTI/PME and TTS
  Calibration feeds share one painter and were labelling their records identically,
  so following a link could scroll the wrong feed. Each labels its own, and a link
  followed in split view lights the pane you are looking at.
- **A link that offers to open a terminal names it as the tab does.** One read
  "the Player tags terminal", which is not what anything is called.

---

## v3.76 patch10
- **The mismatch warning is back, and now catches the case it was built for.** A
  draft model or vision projector built for a differently sized model is marked
  yellow before you pick it. The previous attempt asked the wrong question - it
  looked for a field that only standalone assistant heads carry, so a whole model
  carrying an MTP head reported nothing and passed silently. It now asks whether the
  file attaches to the model at all: a projector always does, an MTP, assistant,
  EAGLE or DFlash drafter does, and a plain draft model does not - the last needs a
  compatible vocabulary and any size at all.

## v3.76 patch9
- **The player's inner monologue is spoken again.** SkyrimNet sends the player's
  thoughts on a voicetype of their own, separate from the player's speaking voice.
  The panel did not know that one was the player's, so it treated every such line as
  a stranger's: the sample check refused it as belonging to somebody else, nothing
  came back, and SkyrimNet reported a failure extracting the audio. The same
  misreading also made the panel refuse to put your character's name on your own
  thoughts.
- **A server that runs out of memory for its KV cache says so specifically.** The
  cache is sized by the context length and its own precision, not by how many layers
  sit on the card - so being told to lower the GPU layers, when the model itself fit
  perfectly well, sends you the wrong way.

## v3.76 patch8
- **The player tags and mood terminal joins the cross-terminal links.** A PTI or PME
  record now carries a **Dialogue** tag pointing at the exact request and reply it
  came from, so a bad tag can be traced back to what the model was actually asked.
  Records with no call behind them carry no tag rather than an empty one.

## v3.76 patch7
- **The jump between terminals now goes both ways, and highlights where it lands.**
  A call in the Proxy terminal that produced reasoning carries a **Thinking** tag;
  clicking it scrolls the Thinking terminal to that call's block and flashes it, so
  you can see which one it was rather than being dropped somewhere in a wall of text.
  The Thinking terminal's **Dialogue** tag still opens the request and reply from the
  other direction.
- If the block has scrolled out of the terminal's window, it says so instead of
  quietly doing nothing.

## v3.76 patch6
- **A record in one terminal can now point at the same record in another.** The
  Thinking terminal's block for a call carries a **Dialogue** tag; clicking it opens
  that exact request and reply. Until now the two terminals had no way to refer to
  each other - each received finished text and could only guess at what it meant.
  The reference is written by the part of the panel that knows what the thing is, and
  the same identifier appears on both sides.
- This is the first stage. Speech, player tags and mood records get the same
  treatment next, and the terminals move onto it one at a time.

## v3.76 patch5
- **Terminate reaches a speech server the panel started before it was restarted.**
  The panel only ever stops a server it launched itself, which is right - one it
  never started belongs to whoever did. But it recognised its own server only for as
  long as the panel stayed open, so closing and reopening the panel left its own
  speech server running and out of reach. A started server now leaves a note behind,
  and the panel takes it back on startup - but only when the process is still alive,
  is still the same executable, and is still answering on the port it claimed. Any
  disagreement and the note is discarded, because stopping something that is not
  yours is worse than leaving one running.

## v3.76 patch4
- **A server that fails to start says why.** The card used to report how long the
  launch ran and quote the last line of the log, which is whatever the engine was
  doing when it stopped rather than the reason it stopped - the actual cause was
  usually one line further up and went unread. The log is now searched for the
  failures llama.cpp names: a draft model or vision projector whose shapes do not fit,
  a projector built for a different width, running out of video memory, an
  architecture or projector type this build does not know, a missing tensor, a port
  already taken, and a rejected flag. A launch that worked says nothing.

## v3.76 patch3
- **Vision projectors are checked against the right number.** A projector was being
  measured on its vision tower's internal width, which has nothing to do with the
  model it plugs into - so projectors built for a 31B model sat unmarked beside a 26B
  one. llama.cpp compares the projector's output width, and so does the panel now:
  the same field, the same answer, for Gemma 4, Qwen 3.6 and 3.8 alike.
- **A plain draft model is no longer marked wrong for being a different size.** Only
  a head that hands a vector back to its host - an MTP or assistant drafter, or a
  projector - has to match the model's width. A whole small model used as a drafter
  needs a compatible vocabulary and nothing else, and was being flagged for a fault
  llama.cpp would never raise.
- **The budget message box is the same width as the settings around it** and grows
  when you click into it, wide enough to read back about a hundred characters, then
  settles again when you click away.

## v3.76 patch2
- **An error in the page now says where it happened.** The panel already recorded
  browser errors, but only the message and a line number from a page it generates -
  which names nothing. The report now carries the functions that were running, so the
  log reads `key is not defined ... in optFor <- paramEditor <- renderSlots` with the
  whole stack beneath it.
- **And it says so on screen.** The error that emptied the Servers page was recorded
  correctly from the first second and sat in the log for three releases, because
  nothing suggested there was a log worth opening. A small notice now appears when
  the page hits an error, pointing at Log > Errors. Click it to dismiss.
- **One broken server card is one broken card.** A single card that failed to draw
  took the whole fleet with it and left the page blank. Each card is drawn on its
  own now: the one that failed says so, the others appear as normal, and the fault is
  reported like any other.

## v3.76 patch1
- **The server cards are back.** A line added in hotfix2 referred to a name that does
  not exist where it was written, so every card threw while being drawn and the
  Servers page showed nothing but its heading and an empty history table. Sorry - it
  shipped in hotfix2 and was there for three releases.
- **The gate now draws the page before it passes.** Every check until now read the
  page as text, which is why a fault that only appears when a line runs went out
  three times. The panel's cards, parameter editor, buttons and thinking terminal are
  rendered against a test server during the build, and anything that throws stops the
  release.
- **One vocabulary for what a terminal is looking at.** Numbers, durations, ports,
  timestamps and character names were each described separately inside four different
  painters, so a term one terminal understood was invisible in another. They are
  described once now, and every quantity reads the same colour whatever unit it
  wears.

## v3.76 hotfix4
- **Your character's name is read from the first line of dialogue, not worked out
  over several.** The panel looked for a party heading, which SkyrimNet only writes
  when you actually have a party - so a lone traveller was never named by it, and the
  panel fell back to waiting until two different characters had spoken to you. That
  took anywhere from a minute to seven. The prompt states who you are outright, and
  the panel reads that instead: the line naming you as the player character, the
  inventory only you have, and the combat assessment. Any one of them names you
  immediately, and a prompt carrying none of them still claims nothing.
- **Character names are coloured in the Thinking terminal.** The list of characters
  the panel has met was only ever handed to the terminals while viewing another
  machine's panel, and thrown away again straight after - so on your own panel the
  list was empty and there was nothing to colour.

## v3.76 hotfix3
- **The reasoning budget message is a setting on the server card.** When a thinking
  budget runs out, llama.cpp injects a line telling the model to stop and answer -
  the Sampler Guide has described it for a while, but there was no way to change it.
  Type your own (`Answer now.`) into the box beside Reasoning budget and it is
  written into the launcher's Generation section. Left empty, nothing is written and
  llama.cpp keeps its own wording, which is what you want unless you have a reason to
  override it.

## v3.76 hotfix2
- **A draft model or vision projector that cannot attach to the chosen model is
  marked before you pick it.** These files declare the width of the model they were
  built for, and a mismatch is fatal - but llama.cpp accepts the pair, because both
  carry the same vocabulary, and only stops much later with a raw assertion that
  names neither file. Non-matching entries are now yellow in the dropdown with the
  reason on hover, a pulsing **Bad LLM Match** label appears if one is chosen anyway,
  and the launch writes the same reason to the log so it sits above the crash rather
  than nowhere. Nothing is blocked - a pairing the panel doubts still launches if you
  want it to.
- **A server that fails to start gives the Launch button back.** The card ended the
  launch only when the server began serving, so one that started and then died left
  the card on "Launching Server..." until the page was reloaded. A launch now ends
  when the slot is confirmed down as well, offers **Launch again**, and has a time
  ceiling under both so it can never stick.

## v3.76 hotfix1
- **The panel works out your character's name properly.** It used to take the first
  thing it saw and keep it: one session read "You are speaking to someone" from two
  different characters and spent the next two hours calling the player *someone*,
  with no way to correct it. A name now has to be shaped like a name, and the sources
  are ranked - a guess from conversation is replaced the moment better evidence turns
  up, rather than locking the answer in.
- **A new source, and the strongest of the automatic ones**: when a line goes out in
  the player's own voice, the panel matches its words against the replies it carried
  and takes the name attached to them. That is a record, not a guess. Speaking into a
  microphone leaves no reply to match, so nothing is claimed.
- **Token counts are back on the payload window** - input, output, and the tokens
  spent reasoning. A server that reports its own reasoning count is quoted exactly;
  otherwise the figure is estimated from the reasoning text and marked with a tilde.
- **Every terminal paints from one table now.** Colours, numbers, units and character
  names were each defined separately inside individual painters, so a term one
  terminal understood was invisible in another. Numbers read coral everywhere,
  including written-out ones, and a character's name is bold magenta wherever it
  appears - drawn from the characters the panel has actually met rather than a list
  hard-coded into each view.
