PandorumLLM v3.80 Beta - local LLM stack for SkyrimNet, all in one folder
============================================================
...
v3.76 patch36: two quiet correctness fixes. A character's line can no longer
be mis-labelled with the player's name in the records and the Audio Cache -
the voice you heard was always right, but three lines in the logs wore the
wrong name on the way through, and the cache learned it. And the panel page
no longer spends most of a second per refresh probing ports that are simply
not running yet - a port found down is trusted to stay down for a few
seconds, and pressing Launch always re-checks immediately.

v3.76 patch37: the thought queue from patch34 is rolled back - it held each new
line until the previous thought finished playing, which slowed whole turns.
Thoughts ride after their own line again, with the Delay field (0.45 s default)
feeding the timer. Remote sessions can now open a job's prompt/reply payload,
and the Audio Cache gained an Audition dropdown to play a clicked take or
sample on the panel PC or the remote one. The hidden voice warm-up now logs as
"Dummy Chunk" with a gold glow (tag strip glows white) so you can see it fire.

v3.76 patch38: broken voice-clip headers can be repaired from the panel. Some
voicetype WAVs claim more audio than the file holds (a data size of 0xFFFFFFFF
from a writer that never finished its header) and the speech engine refuses
them. Under Local Voice Clips, Scan Voice Folder counts the damage and Repair
Headers fixes it in place - each file is copied into _header-backup first, and
only the two size fields are rewritten. Honest files are left alone.

v3.76 patch39: the provider drop boxes on the Live Network now explain
themselves. The empty box reads "drop any number of providers here to link to
server" in the panel accent with a soft glow, centred in the box, and the hint
disappears the moment the first provider lands. Several new users read the
empty box as a single slot - it never was.

v3.76 patch40: the small launch terminal at the bottom of each server card is
gone - it scrolled too fast to read. In its place the VRAM report is now a
stacked list, one row per allocation llama.cpp names (weights, KV cache,
compute, and any drafter/projector/recurrent/MTP buffers), with a total, host
spill and free-at-load underneath. During a launch each core row walks its
lifecycle: queued, then allocating..., then the size - each sized row with a
chip saying where it landed (VRAM, GPUs named on a split, or RAM for host
buffers). A load that dies keeps what landed, dashes what never came, and
marks the failed row OOM in red when the log shows the engine ran dry. The
list persists after the attempt, and the last launch's numbers show as soon
as the page opens.

v3.76 patch41: tag switches are the whole truth - anything you enable on the
tag boards is offered to the player tagger and the mood reader and can reach
the engine, anything you click off leaves their prompts at once, and nothing
is ever removed behind your back again. On the TTS page the paragraph-long
setting titles (Voice Prime, Runaway Handling, Chunk Size, Kept Takes and
friends) folded into short titles with the usual ? chip - hover for the full
story.

v3.76 patch42: the Meta next-speaker guess must now name someone actually in
the scene - a wrong target or a pasted prompt example is refused, said in the
TTS terminal, and the first-chunk tag strip covers that switch instead. Also
audited on request: the x-realtime number is the winning synthesis attempt
against audio length and nothing else - tagging, mood, estimates, holds and
discarded retries are all itemised separately and never inside it.

v3.76 patch43: the VRAM report is anchored to the launch you pressed - no more
stale sizes flashing from the previous log before the new one exists; the
queued/allocating lifecycle now shows from the first instant. Newer engine
builds are parsed fully: the vision projector (clip loader), its compute
reserve, and the drafter's summary-form caches all count now, so totals are
honest. "Host spill" is renamed "Host buffers" and every row has a hover
explanation - host buffers are the engine's own pinned-RAM staging, not VRAM.

v3.76 patch44: the VRAM report no longer flashes at all. It is now its own
persistent element - built once per server, carried across every card repaint
as the same node, and updated row by row in place, with a brief accent pulse
when a value lands. The first poll after a launch press also waits for the
launch call itself, so it can never race ahead and read the previous log.

v3.76 patch45: the Launch/Stop/Terminal row on server cards is now its own
persistent element like the VRAM report - patched in place with a soft
transition instead of being rebuilt, so nothing around it flashes on state
changes. The stuck-button protection from earlier is unchanged; a failed or
hung launch still frees the button by itself.

v3.76 patch46: fixes the missing Launch/Stop/Terminal buttons introduced by
patch45. The buttons had their own element but the server cards never gave it
a place, so the row came up empty after the first redraw. The card renderer
now puts the control row and the allocation report back in place itself every
time it draws. Correct states, still no flashing.

v3.76 patch47: adds Fish Audio S2 Pro as a second one-click TTS beside Higgs.
Pick it in the dropdown at the top of the TTS page and press Install - the panel
fetches the latest audio.cpp and the Q8 model (about 6.3 GB) into its own folder
and fills in every field. Switching between Higgs and Fish now selects that
model's own Q8 file and reference samplers for you. Note that Fish reads none of
the emotion tags Higgs does - with Fish selected the panel strips tags instead
of sending them, so nothing is ever read aloud as markup.

v3.76 patch48: llama.cpp now updates itself from Folder Setup. Two buttons sit
beside the llama.cpp folder - one per Windows CUDA line - and each fetches the
newest release plus its matching CUDA runtime DLLs and unpacks them into that
folder, without deleting anything already there. Use CUDA 13.3 on RTX 20-series
and newer; CUDA 12.4 only on GTX 10-series, Titan V and older, which CUDA 13 no
longer supports. Stop your servers before updating and restart them after.

v3.76 patch49: six more TTS models install in one click - Moss TTS v1.5 Local
GGUF, DramaBox, DotTTS-MF, OmniVoice, Chatterbox and PocketTTS-ENG - beside
Higgs and Fish. Each shows its download size in the dropdown and its licence in
the confirm. Chatterbox is voice-clone only and says so. Tags now follow the
model rather than the engine: Higgs reads the panel's tokens, MOSS keeps its
pause marker either way it is run, and the rest have tags stripped instead of
spoken. Samplers use each model's published defaults, or stay blank so the
engine's own defaults stand.

v3.76 patch50: the TTS page replaces the single "Installed" badge with two
honest lines - one for audio.cpp (missing, not audio.cpp, too old, update
available, or current) and one for the selected model (present, missing, or
the wrong model). The model check opens the GGUF header and reads what the
file declares itself to be and how it is quantised, so a renamed model is
caught rather than trusted. Each line has its own button, and audio.cpp is
only downloaded when it actually needs to be - not once per TTS. audio.cpp
0.6 or newer is required. The dropdown now reads "Name [Audio.cpp]" and shows
in yellow any model you have not downloaded yet, with its size.

v3.76 patch51: models trained with sliding-window attention are now recognised -
the window shows as "SWA Meta: N" beside the Built-in MTP label, and the second
KV cache llama.cpp builds for those layers appears as its own SWA cache row in
the allocation report. That report is now colour-coded: VRAM green, CPU yellow,
system memory plain - red only when weights actually spilled into it, with a
"RAM Offload Detected!" line under the report. Fixes a bug where memory-mapped
weights sitting in system RAM were counted as VRAM. Adds a Full-size SWA cache
setting for when the server keeps re-processing whole prompts.

v3.76 patch52: the dashboard now reads tags as stage directions - *amused*,
*laughs*, *whispering* - instead of showing raw markup, in whichever form the
tag arrived. Plain dialogue in brackets is untouched. Also extends the test
suite so every TTS in the panel is checked for correct tag handling rather
than every kind of TTS, which means models added later are covered from the
day they arrive.

v3.76 patch53: the panel now finds a TTS model anywhere in your models folder
and identifies it by its own header rather than by where it sits or what it is
called - fixing a case where a SenseVoice file in the Higgs folder was read as
the Higgs model. Both model pickers colour their entries: green for a file that
says it is the model you need, red for one that says it is something else, and
plain for a file that declares nothing recognisable. The SenseVoice list stops
hiding voice models by a fixed list of names.

v3.76 patch54: every TTS now gets the tags it can actually perform, written the
way that model reads them - Higgs its control tokens, Fish free-form bracket
words, OmniVoice and Chatterbox only their published lists, MOSS its pause
marker, and DramaBox its own script format with speech in quotes and
directions outside. DotTTS and PocketTTS perform none, so none are sent. The
tag boards and the PTI/PME prompts now populate from the selected TTS and swap
when you change it.

v3.76 patch55: corrects three things against audio.cpp's own documentation.
Chatterbox is sent no tags - audio.cpp gives it no tag interface, so the tags
patch54 added would have been read out as words. Higgs now defaults to Top K 30
instead of 50, which audio.cpp ships because it is less prone to ending a clip
early; existing settings are untouched. Fish Audio's samplers are left blank so
the engine's own tested defaults apply.

v3.76 patch56: fixes the panel failing to recognise audio.cpp models. Every
audio.cpp model file calls its architecture "audiocpp" and names the actual
model family in a separate field - the panel was reading the first, so every
file looked identical, the installer said "it calls itself audiocpp" about
models that were present, and the model list showed no colours. It now reads
the family field. The four TTS folders (audio.cpp, Models\TTS, TTSaudio and
Sample Vault) are now created and owned by PandorumLLM - still shown on the
page, but read-only, since a wrong folder was behind every "it cannot see my
models" report.

v3.76 patch57: the model lists now colour every entry - a model belonging to an
engine the panel does not carry is red, not white, because the file did say what
it was. White now means only that a file said nothing about itself. The
SenseVoice list shows speech models in red rather than hiding them. Every model
read is recorded in model-kinds.json with a plain reason for how it was
identified, and that file is now one readable record per line. The TTS page
loses the audio.cpp releases link and its update button (the check is
automatic), and "Repaired Sample Vault" becomes "Sample Vault".

v3.76 patch58: TTS models get their own record file, tts-model-kinds.json, with
the family each file declares, its quantisation and why the panel reads it that
way - one record per line, like the language-model file. Fixes the TTS dropdown
saying "not downloaded" above an installation line saying "present": both now
search the whole models folder. Safetensors folders are coloured honestly
instead of being left white. Both record files rebuild once on this update.

v3.76 patch59: fixes servers failing to launch with "error: invalid argument:
off". The Full-size SWA cache switch added in patch51 was written into the
launcher with a value, but llama.cpp takes it with none - so llama-server
refused the argument and exited before loading. It is now absent when off and
written alone when on. SWA Meta also reads as a label beside Built-in MTP head.

v3.76 patch60: a freshly opened TTS page no longer claims audio.cpp and your
model are missing before it has checked - it says "checking..." until it knows.
Fixes MOSS-TTS-Nano being recognised as MOSS-TTS-Local: a model that states its
family is now matched exactly. Safetensors folders are identified from their
config.json and coloured like any other model. The TTS record file now states
the panel's own verdict for each model, in words.

v3.76 patch61: the tag boards are now genuinely per-TTS - Fish and DramaBox were
being offered Higgs' whole tag list, and each now gets what its own documentation
shows. Tags are translated into each engine's published words where one means the
same thing. Fixes the installation status sitting on "checking..." until you
changed pages - it is a live element now that retries by itself. Models with no
audio.cpp family tag, such as an LTX quant, are identified from their header
instead of showing uncoloured, and the TTS record file covers every file the
TTS pages look at.

v3.76 patch62: fixes some Q8 models being reported as F32 - the panel now judges
a file by its quantised tensors rather than by whichever type holds the most
data, and records the full tensor breakdown so the reading can be checked. A
model it cannot identify is named outright. The installation lines read as
labels. And starting TTS with the wrong model selected now asks first, naming
the file, the setting to change and the models on disk that would fit - with
the option to force the launch anyway.

v3.76 patch63: fixes the installation line reporting on the wrong file - with a
TTS selected whose model is not downloaded, it described whatever else was
selected, so the dropdown and the line appeared to contradict each other. The
wrong-model message now says what a file actually is instead of printing a
technical value every audio.cpp model shares. Empty tag groups say "No Emotion
Tag Support" rather than showing an empty box, and the two installation
headings read as grey labels in white text.

v3.76 patch64: the wrong-model message names the selected TTS before saying what
does not match it. A tag group with no supported tags shows as a plain label
instead of an empty box, tag boxes use the same dark glow as the rest of the
TTS page rather than a grey outline, and the installation labels are padded to
match the status labels beside them.

v3.76 patch65: every TTS now remembers its own settings - chosen model, both tag
boards, tag limits, and whether PTI and PME are on - so switching between them
no longer disturbs anything. A TTS with no tag support turns the taggers off by
itself and gives your settings back when you switch to one that has it. When a
TTS has no model chosen, the panel picks the best quantisation on disk (Q8, then
Q6, Q5, Q4), never replacing one you chose yourself, and says plainly when no
model is available or an incompatible one is selected.

v3.76 patch66: downloading a TTS model now shows what the repository actually
holds - every .gguf with its quantisation and size, the best one marked
Recommended (Q8, else Q6, Q5, Q4). Fixes Rescan sticking on "scanning" when a
redraw did not land, and adds "Auto select model" to the wrong-model message,
which picks the right file for the selected TTS.

v3.76 patch67: fixes the installation panel switching to a different layout when
it found audio.cpp and models on disk that were not selected - an older screen
that predated the status lines and had none of their improvements. The row is
now the same in every situation, with "Use what is already here" as one more
button on it. Starting TTS with no model selected now says "No compatible TTS
model selected" instead of describing it as the wrong model.

v3.76 patch68: a TTS with no model on disk now shows "(none selected)" instead of
holding on to a model belonging to another TTS. The download dialog is much
simpler - it names the TTS, lists the files with quantisation and size, marks
the recommended one, and lets you pick more than one; where the files come from
and their licence are a single line beneath.

v3.76 patch69: the installation row has two buttons again - install or update
audio.cpp, and download the model, the latter only while the model is missing.
The "Use what is already here" button and everything behind it is gone, since
the panel now handles all of that by itself. The download list is plain text
with a glow rather than boxes, picked files show a green tick, and the Download
and Cancel buttons glow accent and red instead of both white.

v3.76 patch70: fixes audio.cpp failing to install or update from release 0.7.0,
which replaced its balanced/fast builds with one build per CUDA version. Both
engines now use the same chooser: it shows CUDA 12.4 and 13.3, marks the one
installed, and asks a second time if you pick the other. llama.cpp's two
buttons become one.

v3.76 patch71: the engine installer for both llama.cpp and audio.cpp now shows a
link to the releases page, the version you have in yellow, the version available
in green, and the two CUDA builds with who each is for. The highlight follows
the text rather than looking like a box. Every button on the panel is now in the
panel's own typeface - buttons do not inherit the page font unless told to.

v3.76 patch72: fixes the llama.cpp installer reporting your build as "b0" when
llama-server was built without git information - it now says "installed, build
unknown". The installer dialog describes each CUDA build in a few words with
the detail on hover, ticks the selected one, and links only the engine name.
On Folder Setup the Install/Update button replaces "Check for update", and the
CUDA descriptions no longer appear twice.

v3.76 patch73: the llama.cpp installer reads your installed version properly -
the build number where there is one, the commit where the build was made
without git history, or the version recorded in the .exe. And it says "Update
llama.cpp" whenever llama-server.exe is in the folder, rather than only when
the panel installed it.

v3.76 patch74: fixes updating llama.cpp against releases that carry no Windows
build - llama.cpp tags version releases such as v0.3.0 whose binaries live in
the numbered bNNNNN releases beneath them. Both engines now look through recent
releases until they find one with a build for your CUDA version, and the
version shown is read from the file being installed rather than the release it
was filed under.

v3.76 patch75: fixes the llama.cpp update sticking on "starting..." and the
finished row never appearing. llama.cpp also gains the status line audio.cpp
has - latest version installed, update available, or not installed - which
keeps itself current and says "checking..." until it knows.

v3.76 patch76: the status lines update the moment an install finishes rather
than up to a minute later. A good verdict - engine up to date, or a compatible
TTS model present - is filled green so it can be seen without reading. The
selected model gets its own label, the two TTS labels are the same width, and
the grey "last installed by the panel" line is gone.

v3.76 patch77: downloads are verified against the SHA-256 the publisher states
and deleted if they do not match; the panel only fetches from github.com and
huggingface.co over HTTPS and refuses redirects elsewhere; the installer
windows show the filename, size and SHA-256 before you press anything; and
llama.cpp installs inside PandorumLLM when no folder has been chosen. The TTS
model line lists the quantisations you have rather than the model name.

v3.76 patch78: a failed install now shows the download address, click-to-copy,
with what to do by hand - for whichever of the three things failed. "Local
Voice Clips" and "Sample Vault" were the same folder and are now one setting
called Sample Vault. The audio.cpp and TTS model labels are the same width.

v3.76 patch79: every folder the panel uses now defaults inside PandorumLLM and is
locked to it. A padlock beside each one unlocks it for editing - asking first,
and leaving your path alone while open - and a revert arrow puts it back, to the
panel's own folder or to any path you have used before, kept as a list of up to
a hundred.

v3.76 patch80: adds auto-sort for the models folder - each model filed by what
its header says into LLM, MTP or Vision, then family and size, so a Gemma 4 31B
lands in LLM\\Gemma4\\31B. Nothing moves until you have read the plan, which
names every file, its destination and the server cards using it; cards are
updated in the same step, and anything whose header does not say what it is
stays where it is.

v3.76 patch81: fixes patch80 and patch79 replacing folder paths you had already
chosen. The folder defaults are defaults again - a folder you set yourself is
kept and marked as yours - and where patch79 already replaced one, it is read
back from the server cards that still name their files by full path.

v3.76 patch82: the installer window says whether a build can be verified - a green
tick with the SHA-256 it will be checked against, or a red cross and no install
button when the release publishes none. Adds n-gram speculative decoding to the
server card: it needs no draft model and runs alongside an MTP head rather than
instead of it.

v3.76 patch83: evens out the spacing of server card rows, and moves the n-gram
settings under Speculative Decoding as "Draftless speculation". Adds an N-gram
template - Dense or MoE fills and locks the three ngram-mod numbers, User hands
them back exactly as you left them - and only the settings the chosen method
takes are shown. Hand-edited launchers keep their n-gram settings.

v3.76 patch84: tidies the Draftless speculation rows to match the rest of the
server card - same label styling, working reference links on every flag, and
dropdowns the same width as the fields beside them. The N-gram template row no
longer shows a flag it does not write, and its first option is now called
"User specified".

v3.76 patch85: the allocation report names the buffer that ran out of memory and
the size it asked for, rather than blaming whichever row came next - a failed
SWA cache used to read as Compute. A launch that fails because the card is full
without llama.cpp saying so is now caught and explained. The report is headed
with the model that was loaded and the time it was launched.

v3.76 patch86: adds logs/allocator.log - every launch records what the engine
allocated and what NVIDIA says is actually resident on the card, with the
difference stated when they disagree, plus a summary of every card and the
servers on it. Fixes the compute buffer being counted twice when a reserve line
repeats, and the report header printing "vhdr".

v3.76 patch87: fixes the allocation report flashing once and vanishing when a
server finished launching. It was being restored only after a round-trip to the
panel, so a card redraw arriving first discarded it; it is now restored the
instant the card is drawn and stays put.

v3.76 patch88: fixes the allocation report stopping half way and staying that
way, or vanishing after a browser refresh. It was refreshed only while the
Servers page was on screen and while the launch was still considered in
progress - but a server answers its port long before its last buffer is
allocated. It now refreshes on its own until the log says loading has finished.
The allocator log also no longer implies a card is empty when the driver does
not report per-process memory.

v3.76 patch89: server card sections fold away - click a group heading to collapse
or expand it, with the arrow showing which way it will go and the choice
remembered. The DRY sampler on a provider now exposes all four of its settings
rather than the multiplier alone: multiplier, base, allowed length and penalty
last-n, each with its own field, blank meaning not sent.

v3.76 patch90: Speculative Decoding and Draftless speculation now fold like every
other section - they were built separately from the rest of the card and had
never been given the folding heading. Draftless speculation moves above
Speculative Decoding, the fold arrow is much larger, and folding is now per
card so collapsing a section on one server leaves the others alone.

v3.76 patch91: the fold arrow sits properly on its heading again - patch90 made it
larger by growing its font, which grew the heading row with it. It is now drawn
large without affecting layout. The DRY sampler window also matches the rest of
the panel: the same dark background, with a soft black glow instead of a grey
border.

v3.76 patch92: fixes the allocation report being blank, frozen, or silent about a
failed launch - it was only asked for while a server reported itself as serving,
which it does not while loading nor after exiting. Every server is now asked
until its report is complete. The fold arrow is drawn properly and no longer
moves its heading, and each server card gains its own Collapse all / Expand all.

v3.76 patch93: the allocation report keeps updating until it is actually
complete, rather than stopping the moment a server reports itself ready - which
happens in the same instant the last buffers are recorded. A report that is
still short now names the log file it is reading and how long ago that file was
written, so a stuck report says where to look.

v3.76 patch94: the allocation report no longer decides when to stop refreshing -
three separate rules for that each left a card stuck half-filled, so it now
refreshes every server continuously. The fold arrow is centred by its heading
rather than positioning itself.

v3.76 patch95: the Draftless speculation settings no longer require a draft model
to be selected - n-gram speculation needs none, which is the point of it. The
allocation report now follows the server log rather than a timer: it keeps
reading until the log says the load has finished, however long that takes, and
each reading adds to what is known rather than replacing it.

v3.76 patch96: an allocation report that stops filling in now says why - whether
the panel cannot find the log, is still waiting for this launch's log to appear,
or the request failed. A working report shows none of it.

v3.76 patch97: fixes allocation reports stopping when several servers are launched
together. The panel accepted only a handful of simultaneous connections and the
page asked for one per server at once, so the browser's requests were refused
and each card kept its first reading. The report now asks for every server in a
single request, so twenty servers cost no more than one.

v3.76 patch98: fixes the allocation report stopping at the weights and going no
further. The panel could not send a report once the server log reached its first
compute buffer, so the browser received nothing and each card kept whatever it
had read moments earlier - small models showed no report at all. This has been
the cause since patch86.

v3.76 patch99: fixes Compute being over-reported when a draft model re-reserved
its graph after a vision model loaded. Adds a working "Free at load" line and a
new "Free before drafter" line - the figure that decides whether a draft model
fits. The report header puts the launch time on its own line, values pulse once
when they arrive rather than continuously, hovering a collapsed heading lights
the whole heading, and the pop-up dialogs now match the rest of the panel.

v3.76 patch100: the allocation report reacts immediately again - launching from
the main Launch button used to leave each card on the previous run's figures,
and the report now follows a load closely enough to show each buffer arriving.
"Free at load" and "Free before drafter" move to logs/allocator.log, where they
belong: they are not allocations, and were being labelled as though they were.

v3.76 patch101: emotion tags are no longer printed in front of spoken dialogue -
they read as a stage direction, as intended, and always reached the voice engine
correctly regardless. Thoughts no longer carry an emotion tag at all. And a
remote view no longer warns "this is a read-only view" over and over: reading a
server's own log is now permitted remotely, as it always should have been.

v3.76 patch102: removes the "this is a read-only view" pop-up. A remote view
refreshes constantly, so the message could arrive faster than it could be
dismissed. The header badge and the hidden host-only controls say the same
thing without interrupting anyone.

v3.76 patch103: audio.cpp 0.7 is now the minimum and the "Audio.cpp Build"
setting is gone - 0.7 dropped the separate Balanced and Fast builds for a single
build per CUDA line, so there was nothing left to choose. An older install is
still detected and reported as too old rather than not found, and the manual
install steps now name the archives 0.7 actually publishes.

v3.76 patch104: fixes the TTS page claiming a llama.cpp update was still running
after it had finished, which hid the audio.cpp status and the chosen model until
the update was dismissed on Folder Settings. A finished update or install now
clears itself when you leave its page; Dismiss remains for clearing it sooner.

v3.76 patch105: fixes three TTS faults. Some models would not start, because the
panel sent a setting no current audio.cpp accepts and the retry that worked
around it only had time to act on small models. Short lines could be cut off
before finishing, because the token limit made no allowance for the fixed cost
of starting a line. And MOSS could fall silent for half a minute between
sentences, because the ten-second pause limit was applied to each pause rather
than to the line.

v3.76 patch106: fixes voices that refused every line. DramaBox and DotTTS do not
accept a temperature setting and the panel sent one to every voice alike, so
every line failed. The panel now notices when a voice says it cannot use a
setting, leaves it out and asks again - the line is still spoken, and later
lines are sent correctly the first time.

v3.76 patch107: adds support for LiquidAI LFM2 / LFM2.5 models such as
Shruti-Soft-2.6B. The server card explains why their KV cache is small for the
context - only some layers attend, and the rest shows as Recurrent state - and
warns that this family reasons because it was trained to, so a provider with
Thinking off may still see it think. The control that works is Reasoning budget
set to a small positive number; zero is the wrong value and the card says so. Two
new server settings are added under Context and cache - Unified KV cache and KV
limit per slot - which decide how the cache is shared between parallel slots.

v3.76 patch108: fixes long lines in the voice terminal printing on top of the
line below - a very long record, usually the banned tag list, was given a single
line of height however many it needed. Long records now fold: the label stays on
its own line with a Show button beside it, and the rest appears when asked for.

v3.76 patch109: the TTS chooser shows a size range for every voice, read from
the real model files, and marks the voices still being worked on as WIP. Titles
no longer repeat the engine name; Higgs is now Higgs v3 and Moss is Moss TTS 1.5
Local.

v3.76 patch110: fixes the new TTS chooser not opening when clicked. It is now
half as wide, sits on its own dark background, and its arrow is larger and lights
up with the title. Hovering the name now lights the letters rather than drawing a
rectangle around them.

v3.76 patch111: the TTS chooser is back on the panel's own menu component and
behaves like every other dropdown. Each voice shows what runs it - audio.cpp or
proxy - its download size, and a WIP mark where one applies. MOSS appears twice
under one name, once for each engine that can run it.

v3.76 patch112: fixes the TTS menu showing empty rows, and restores their height.
Adds a start/stop button beside the TTS chooser - a triangle to start the voice
server, a square to stop it - and turns "Revert to default TTS settings" into the
same circular arrow the folder rows use, with its wording as a tooltip.

v3.76 patch113: fixes the download sizes in the TTS menu being missing until a
voice was selected - they are read when the panel starts. The menu labels lose
their boxes, sit on the middle of each row and carry a soft glow, voice names are
bold, and every dropdown on the page now uses the same darker field.

v3.76 patch114: the TTS menu and its field now share one colour, and menu labels
sit centred in their own boxes. The proxy MOSS entry is a different model from the
audio.cpp one and now says so - it is listed as Moss TTS 1.5 Delay, the larger
model the separate MOSS server loads, with its size read from the publisher's own
file listing.

v3.76 patch115: the TTS menu field is now the same dark as the menu itself. Adds
better support for LFM2 / LFM2.5 models - the panel reads the model file's own
layers and reports how many use attention, such as "3/16 layers attend", which
is why these models need so little memory for their context. Mixture-of-experts
files are recognised too, from the file rather than from a list of names.

v3.76 patch116: the model list on a server card shows each file's architecture
before its name and what it is for after it. Fixes models being wrongly labelled
as having built-in reasoning - the panel now reads each model's own chat template,
and asks llama.cpp directly where a server is running. The card also reports
expert counts, the attention ratio, rope scaling and the full-attention pattern.

v3.76 patch117: fixes the model menu labels - no architecture was shown, vision
projectors were labelled as language models, and a model with a built-in MTP head
lost its own label. Adds "Drop the thinking primer": some models open every answer
with a think tag written into their own chat template, and this writes a copy of
that template without it. Your model file is never changed. Off by default.

v3.76 patch118: the architecture label shows the family and nothing else - Gemma 4
rather than "Gemma 4 assistant (MTP drafter)", and Mistral rather than "Llama
family". Fixes drafting heads and vision projectors being called dense. A model
that does not fit shows a warning symbol with the reason as a tooltip, and the
chosen model is shown by name alone.

v3.76 patch119: fixes model menu labels - models with a built-in MTP head were
also labelled as drafters, some drafting models were offered as ordinary language
models, mixture-of-experts variants lost their family name, and a model that does
not fit its server showed two warning symbols.

v3.76 patch120: files that are not language models are recognised as such - an
image model in your models folder was being offered as something to load, and is
now marked "other". A drafting head or projector with no family of its own can be
matched to the model it belongs with by comparing several declared figures; the
match is marked with a ~ and only offered when exactly one family fits.

v3.76 patch121: vision projectors are matched to the model they belong with, by
the width they project into, shown with a ~ to mark it as inferred. Models that
predict ahead now carry one label instead of four - whether the file is a model
carrying a head or the head itself is already shown beside it.

v3.76 patch122: fixes decimal values read from model files being stored as
meaningless whole numbers - your remembered model details refresh once on the next
start, and the panel now says on every start whether they were current. Fixes
drafting heads not being matched to the model they belong with. A model with a
built-in prediction head reads "built-in MTP"; a file that is one reads "MTP".

v3.76 patch123: fixes the panel not starting after patch122 - a message added in
that patch was written at a moment when the panel could not write anything, and
startup stopped there. It is now sent once startup is safely past that point.

v3.76 patch124: vision projectors and drafting heads are matched to their model
far more often - where two model families share the same size, the folder a file
is kept in tells them apart. A drafting head whose model cannot be identified now
shows what kind of head it is. Built-in MTP is yellow and vision is cyan.

v3.76 patch125: fixes the wrong values corrected in patch122 never actually being
refreshed - the remembered model details are re-read once on the next start. The
"dense" label is now white, and a model file that is itself a prediction head is
shown in magenta.

v3.76 patch126: fixes model details that are stated per layer never being read, so
several families showed no attention ratio and could not be matched to their
drafting heads. Fixes models whose thinking cannot be switched off showing no
label. The Sampler Guide is broken into paragraphs and no longer names particular
model makers, and a dropdown now opens at whatever is already chosen.

v3.76 patch127: PandorumLLM can update itself. When a newer release exists the
version button says "Click to Update"; the panel downloads it, checks it against
the published checksum, and shows what it found before anything is touched.
Applying it closes the panel, replaces the files and starts it again, and the
page reconnects on its own. Every file is checked as it is written and the old
ones are kept in logs - if anything does not match, everything is put back.

v3.76 patch128: fixes the model list being able to fail to load and never
recover, and stops the panel calling a model "not in models folder" while it is
still reading that folder. PandorumLLM now shows what it is doing while it starts
up, step by step, and opens when it is genuinely ready. Fixes the model menu not
being draggable past a set height. Higgs v3 is now called Higgs TTS 3.

v3.76 patch129: fixes no models loading at all - a model file that states a
setting once per layer rather than once for the model stopped the folder scan
dead, and with it every model in the folder. The startup screen now shows the
folder scan counting up, so a long scan reads as progress rather than a pause.

v3.76 patch130: fixes the panel reporting its own files as changed after an
update - the update was not installing the file list it checks itself against.

v3.76 patch131: fixes thinking settings having no effect - servers were started
without --jinja, which llama.cpp requires before a model's own chat template and
its thinking switch are used at all. The sampler order is now stated explicitly
rather than left to a default. Opening a completed generation shows the samplers
it actually used, coloured by who chose each one. All five DRY settings are now
in the Sampler Guide.

v3.76 patch132: fixes the updater not finding the checksum published with a
release and reporting that the release had none, and fixes a failed update check
being remembered for six hours so trying again changed nothing. The update window
now matches the llama.cpp and audio.cpp ones and shows the checksum it verifies
against. Everything the updater does is written to logs/update.log.

v3.76 patch133: the update window is now the same as the llama.cpp and audio.cpp
ones - what is installed, what is available, the file, the published checksum,
then Update and Cancel - and it updates itself as the download progresses. Any
published release can now be installed, not only ones carrying a file manifest;
every file is still verified against the checked archive.

v3.76 patch134: fixes the Update button in the update window doing nothing and
its buttons flickering on hover - the window was redrawn from scratch every half
second and now updates in place. The whole update runs inside that one window:
download, checks, closing, swapping and restarting are each shown as they happen.

v3.76 patch135: updating is one press - download, checks, closing, swapping and
restarting run through without asking again, with the bar and the stage list
showing each as it completes. Fixes the browser asking "are you sure you want to
leave?" after an update or on first start, the welcome screen appearing before
the panel had finished starting, terminal windows flashing during an update, and
the update window's buttons flickering on hover.

v3.76 patch136: the version button says "Checking for updates" while it asks
GitHub, and clicking it during that time explains what it is doing. The
update-test build says which patch it was made from.

v3.76 patch137: fixes updating opening a second panel tab - the panel restarts in
place and the page you started from reconnects. The update window stays open
when it finishes and reports the result: which stages completed, and Update
successful or Update failed with the stage it failed at and why.

v3.76 patch138: fixes the update window staying at "Closing the panel" after a
successful update - each stage of the swap is now reported from what the update
actually recorded, and the window recognises the restarted panel by what it
serves. Fixes updating to an older release being reported as failed when it
had succeeded.

v3.76 patch139: the update now clears up after itself - the files it replaced, its
download folder, and any leftovers from an earlier attempt - shown as a final
stage, Clearing the old files. The backup of your previous version is kept.
Fixes each update leaving .old copies in the folder. The update log names the
files it wrote.

v3.76 patch140: Remote Access shows the panel's version status - green when it is
the newest release, yellow when an update is available - without being able to
start an update; that stays on the host. The Permission Tree describes the
updater. Only the latest rollback backup is kept after an update.

v3.76 patch141: failed updates and failed startup steps now appear on the Log
page as errors, with stable codes (UPD-E01 to UPD-E10) that can be quoted.
Internal: the checking system that guards each release was strengthened.

v3.76 patch142: internal - the release checks now look for the kind of lock
re-entrancy that once froze the panel. No user-facing changes.

v3.76 patch143: Thinking: trained - when a model returns reasoning although
thinking was switched off, the panel notices, marks that model, and shows a
yellow warning beside the Model selector, the Reasoning setting and the
provider's Thinking toggle. The reasoning a server returns is captured whether
or not thinking was on. Shorter tooltips on the thinking chips.

v3.76 patch144: the blue [--flag] buttons now open the Sampler Guide's card right
where you are, as a pop-up, with a Sampler Guide Page button at its foot for
the full page. Fixes the four DRY entries in the guide showing no range line.

v3.76 patch145: internal - the release checks were made independent of one
another. No user-facing changes.

v3.76 patch146: a search field above every model picker on the server cards
(model, vision projector, draft model). Type to narrow the list to matching
names; the menu shrinks to fit; Enter takes the first match.

v3.76 patch147: the user guides catch up with the panel - the main guide's steps
say to press Install llama.cpp and check the preset folders, every step has a
hover tip, and clicking a step highlights what to press. The TTS guide gets an
audio.cpp tab with the one-click install and every voice model the panel can
run on it.

v3.76 patch148: the model picker's search field is reworked - it appears above
the bar only while the menu is open, in the menu's own style. Fixes voiced
thoughts never showing their realtime ratio, and spoken lines appearing out of
order in the proxy terminal. The payload card names the model.

v3.76 patch149: combat lines are voiced properly - a short combat line had lost
its speaker and waited on voice priming. Reasoning a model produces with
thinking off is kept out of what SkyrimNet receives and shown as reasoning;
such a model is marked "thinking: trained". Model files are re-read once.

v3.76 patch150: embedded actions show in the proxy terminal under the reply that
chose them. The TTS guide is one audio.cpp guide with every voice model listed.
Main guide boxes show their tasks as centred bullet lines and keep their tooltip.

v3.76 patch151: a model added to your models folder is picked up straight away -
the panel checks when you open a model menu or the Servers or TTS page. A model
file that cannot be read is tried five times, then reported with what was
missing and why.

v3.76 patch152: the startup screen counts as it goes instead of jumping. The
model search field is the menu's dark grey and the menu matches the selector's
width. TTS guide links the model repository and names your own models folder in
every download command.

v3.76 patch153: the terminals remember where you left off. The version button
tooltip tells you which version you are running and whether an update is
waiting. The main guide's last step shows the Launch LLM button itself, ready
to press. Toggle switches are identical everywhere.

v3.76 patch154: fixes an NPC speaking the "4.09x" speed reading of their own
thought when it was clicked to replay. The proxy terminal shows each voice tag
by the model's own word, so the line matches the payload. Embedded actions read
simply as "Embedded action: Name".

v3.76 patch155: fixes every server card claiming its model had no thinking
switch - the panel was asking llama.cpp for a detail it no longer reports. The
proxy terminal shows every voice tag a reply carried. The terminal remembers 150
request/reply pairs and keeps them across a restart; note that this writes the
full request, including the system prompt, into your log folder.

v3.76 patch156: if spoken lines are not being shown in a terminal, it now says so
and names the switch that shows them. Embedded actions show what was chosen
again, with the label plain and the action in cyan.

v3.76 patch157: fixes the thinking label changing when a server started - the
panel now uses the model file's own answer whether or not the server is
running. Fixes settings silently failing to save when two changes landed at
once.

v3.76 patch158: spoken chunks are synthesised in the order they arrive, so a
reply's sentences appear in the order they are said. Speech timings no longer
include waiting behind another chunk. The TTS terminal records every speech
request as it arrives and says what asked for it.

v3.76 patch159: voice tags now survive on the first line after a speaker switch.
The panel used to strip them and warm the voice with a hidden chunk first, which
cost about 1.5 seconds per switch and could drop a line entirely. Voice Prime is
gone from TTS settings.

v3.76 patch160: fixes a server refusing to start when Built-in MTP was still
selected from a previous model that had one - the panel now warns and launches
without it. Fixes a doubled emotion tag on player lines.

v3.76 patch161: adds a provider for the SkyrimNet-Relationships plugin. It
arrives unallocated like any other - place it on a server from Live Network,
and point the mod's snrom_background variant at it.

v3.76 patch162: terminal colours are defined in one place again - the newer line
types added over the last few patches now take their colours from the same
table as everything else.

v3.76 patch163: each log entry sits in its own panel in the TTS, Thinking,
PTI/PME and Calibration terminals. The Proxy terminal groups a record with
everything belonging to it and alternates the shading between records. The
payload card shows token counts as one field.

v3.76 patch164: each panel in the terminals glows in a colour that says what it
belongs to - the provider's colour, or a spoken line's or a thought's. Panels
fade in over half a second, as does a server card's allocation line.

v3.76 patch165: fixes Split View throwing on the Thinking feed. Panels are as
wide as their text and appear in every terminal, and their glows show the
provider colour again. The TTS terminal keeps one log per run like the others,
so after a restart a record's spoken lines come back with it.

v3.76 patch166: a record is one panel again, with everything belonging to it
inside. The Thinking, PTI/PME and Calibration terminals no longer break a single
record into a panel per paragraph, and their panels take the provider colour.
Panel edges glow inward as well as out.

v3.76 patch167: the embedded-action link mark is accent coloured and glows,
like every other link. Panel borders are drawn half as thick.

v3.76 patch168: embedded action rows now match the rows around them - the branch
line connects and glows, the icon sits in the same column, and a long line wraps
under its own text. Numbers and quotes inside action rows take the same colours
they do everywhere else.

v3.76 patch169: fixes the whole terminal flashing whenever a single new line
arrived. Only a genuinely new panel fades in now.

v3.76 patch170: reference chips now work - clicking [Thinking] or [Speech] opens
that terminal and lights the matching block. The Show link on a long line stays
open. A model added while the panel is running appears in the server card menus
without a restart. Folder Settings is tidier.

v3.76 patch171: a model added while the panel is running now reaches the server
card menus by itself. Panel borders are a single soft line again.

v3.76 patch172: models are remembered one entry per file - a model that changes
is read again, one that is deleted is forgotten, and one still being copied in
is picked up once the copy finishes. Fixes the model menu closing itself and
the [Thinking] chip not taking you anywhere. A record too wide for the window
shrinks to fit.

v3.76 patch173: a model still being copied in is no longer scanned - the panel
checks the file against the size its own header declares. The model menu no
longer closes under you when the list changes while it is open.

v3.76 patch174: a model that was still being copied when the panel looked at it
is read again by itself once the copy finishes - no restart needed. Files the
panel could not read are marked TBS. Folder Settings has its short titles with
the explanation behind a hover.

v3.76 patch175: a model finishing its copy is picked up the next time you open a
model menu, and the menu no longer closes when the list refreshes. Folder
Settings titles are white with the explanation behind the ? chip. Records too
wide for the terminal shrink to fit.

v3.76 patch177: fixes the terminals going blank as soon as a record carried an
embedded action. Nothing was lost - the logs were intact and only the drawing
failed - but no terminal would show anything until the panel was restarted.
The voice-model list (tts-model-kinds.json) no longer ships: it is a record the
panel builds on your own machine, and the copy in every release so far was the
developer's, full of entries for temporary test files. An update can no longer
overwrite your own copy, the start-up check no longer reports it as a changed
file, and an install that already carries those entries drops them the first
time the list is read. The error notice at the bottom of the page showed "&lt;-"
between function names instead of "<-". patch175 and patch176 were built but
never published; everything in them is in this release.

v3.76 patch178: fixes the model scan for a file copied into the models folder. Windows
gives a copying file its full size at once, so a scan that overtook the copy read
zeros and called the file a complete "other". A header that ends early is now an
unfinished read: the file is marked TBS in red, a file another program still holds
open is never counted as failed, and the folder is watched so the file is re-read
and the list updated by itself once the copy finishes. The model menu no longer
closes while the list refreshes, and a re-read model updates its row. The player's
spoken line now hangs under the PTI record that tagged it, with one emotion tag
instead of two. The TTS Calibration terminal is boxed per record. The User Guide's
step-7 note shows only while the step is still to do. Log > Errors now says what
happened and what to do in plain words, names a fault that came from a browser tab
still running an older page, and lists a server launch that never came up and an
install whose files differ from the release.

v3.76 patch179: fixes spoken lines landing under the wrong call in the Proxy terminal,
or standing on their own. They were placed by the clock, and SkyrimNet voices a
reply's sentences over tens of seconds - some before the call has finished. A line
is now placed by its words: the panel keeps every reply from its first word, finds
the sentence in it, and hangs the line under that call with the speaker the call was
for. A line the player typed is filed under the PTI call that tagged it, first and
always. Calls with a thought and no speech: SkyrimNet never asked for those sentences,
and once a reply is two minutes old the panel says which - under the response in the
call's payload, and as a note in the TTS terminal. The "previous session ended" line is no longer drawn as a box, and every
Folder Settings title has its explanation behind the ? chip, with the launcher sweep
a title and a Sweep now button.

v3.76 patch180: the proxy is a proxy again - what llama.cpp sends is what SkyrimNet
receives, byte for byte. Since patch149 every reply chunk of a provider with reasoning
off was rebuilt on its way out, and the leading-thought reorder had spread from
Dialogue to Combat; the official 3.76 rewrote only a Dialogue reply that opened with a
thought. Both rewrites are now off by default, named on Proxy Setup (Response wire).
The panel's record-keeping now runs after the reply has been sent and the connection
closed, never before. A request that fails inside the panel answers 502 with the
reason and appears on Log > Errors with its port, instead of a dropped socket and no
line. The "previous session ended" line is back in every terminal, and the
"Allocated to..." line no longer flashes while a server loads.

v3.76 patch181: a model of a new architecture now shows its family in the menu - the
name is read from the architecture string itself (spark2_5 reads Spark 2.5) when the
panel's table has no entry. Spoken lines no longer land under the reply before the one
they came from: a request SkyrimNet sent twice and dropped left a half-read ghost of
the reply in the speech ledger; a dropped stream is now recorded and closed, and a line
goes to the reply that contains its words.

v3.76 patch182: terminal boxes shrink to fit a narrow window - a record row no longer
wraps into a jumble, the box scales down with its contents; spoken lines still wrap.
Full window lands on the bottom row. The text after "Embedded action:" is cyan with
the usual colours over it, the action name wears the highlight with a slight glow, and
the same on an action row. Box gaps are the same on every side for one row or many.

v3.76 patch183: spoken lines are filed by sentence - a short "Ha!" or "Oh!" no longer
lands on whichever reply contains those letters inside a word, and a line no reply
holds stands beside the clock instead of being guessed at. The player's lines, with
PTI off, bundle into one box when they follow within ten seconds. A model of a new
architecture shows its family in the menu even if it was scanned before the panel
could name it. Full window keeps its scrollbar visible. The payload viewer shows a
JSON reply key by key with real line breaks and each {% block %} on its own line,
painted with the same colours as the Thinking terminal.

v3.76 patch184: fixes spoken lines landing outside their reply's box, and two replies'
lines merged into one. The cause was the panel's own since patch179: every spoken line
named a record id one short of its record, so the terminal fell back to the clock,
and it read that reference as the speaker, so lines of different characters spoken
within seconds were treated as one run. Patch181's explanation - SkyrimNet sending each
request twice - was wrong.

v3.76 patch185: patch184 had dropped the record id from every Proxy terminal row, so a
click on a title opened Live Network instead of the payload and spoken lines had no row
to hang on; fixed. Folder Settings has one "Sweep launcher folder" button with its
explanation on hover.

v3.76 patch186: action rows are back - SkyrimNet's action providers answer in the
ACTION: Name PARAMS: form now and the panel read the old JSON. A Targets switch beside
Actions shows the row of the Meta call that decides who speaks next. The terminal keeps
250 payloads. Folder Settings draws a rule between fields with equal air above and
below, and the llama.cpp row has Open folder. The "launch did not finish" errors that
appeared at every start-up came from the previous session's logs and are gone. The
payload viewer shows a reply as three paragraphs: spoken, thought, action. Spoken lines
no longer go missing from a box whose reply had them: the terminal now reads every
spoken line back to the oldest record on screen, not the last 400 lines of the TTS log.

v3.76 patch187: the action row is the action evaluator's alone again - patch186 had
given one to every reply carrying an ACTION line, doubling the embedded row. The
target selector's row is always Speaker > Listener whatever shape the model answered
in, the player wears the player's name, 0 reads "no one speaks next", no emoji. Targets
has a switch in the Options menu beside Actions. Inside an action's brackets the values
are highlighted, numbers in the number ink, keys in the tag ink, symbols white.

v3.76 patch188: the (realtime) figure on every spoken and thought line in the Proxy
terminal is back - patch186's deeper TTS read had kept the spoken lines alone, and the
figure is read from the line that follows each one.

v3.76 patch189: a restart is not amnesia - the Proxy terminal shows the previous
session's spoken lines with their realtime figures, the thought voice reads the
name-to-sample ledger from disk instead of saying no voice was learned, and the
generated line audio lives beside the panel's logs (tts-audio) keeping 250 lines by
default (setting ttsAudioKeep) so a line from an earlier session can still be played.
The Thinking chip on a record row is centred.

v3.76 patch190: patch189 showed no spoken lines when the previous session ended in the
evening and the new one began in the morning - a clock was compared across midnight;
fixed. Every kept Dialogue or Combat record now writes its own spoken sentences under
itself, so the lines are in the box whether or not TTS voices them; a voiced line takes
the written row's place with its (realtime) figure, an unvoiced one stays plain.

v3.76 patch191: the panel checks its own work - a new "audit" source under Log > Errors
files a finding where a fault would otherwise show as a silent empty (a reference
naming no record, a row without its id, a reply that yields no row, a terminal with
speech records and no spoken lines). The same fault is written once, then with a count.
Log > Diagnostics reads the session back - records, references, rows, voiced lines,
the speech numbers, the errors - with a Copy report button for bug reports.

v3.76 patch192: a self-test - "Run self-test" on Log > Diagnostics, or
PandorumLLM.exe --selftest from a console - exercises the panel's own mechanisms on
this machine in a scratch folder it removes afterwards: the folder watcher, the busy
file probe, a copy in progress, settings under contention, log rotation, the DLL swap,
the audio folder, accented names, PowerShell, the GPU query. The report is saved beside
the logs; paste it with a bug.

v3.76 patch193: patch192's Run self-test could freeze the whole page until a browser
refresh - one request that never answered held every redraw. No request can hold the
page any more (a call unanswered for 90 seconds is abandoned and reported), and the
self-test runs in its own thread while the page polls for the report. The Proxy terminal's
boxes are right again: spoken lines under their own record, no Request chips on written
rows, no doubling, a player's line never swallowing a reply's rows.

v3.76 patch194: one audio system - the Proxy terminal replays the take kept in the Saved
Audio Folder (patch189's logs\tts-audio folder is gone; delete it yourself), Kept Takes
per Character defaults to 250 with the reason on its tooltip. When the Providers filter
hides every record the terminal says so and files an audit finding. The sample-length
setting has a tooltip.

v3.80: the release that rolls up patches 181-195 of v3.76 Beta. The Proxy terminal reads
the record (every reply's sentences under it, voiced lines replacing them with their
realtime figure); a restart is not amnesia (previous session lines, the voice ledger and
the takes survive); the panel checks its own work (Log > Errors "audit", Log > Diagnostics
with a copyable report, Run self-test); no updater while its own server is up; no request
can freeze the page; action and target-selector rows in SkyrimNet's current form.

v3.76 patch195: no updater while its own server is up - the llama.cpp updater waits for
the llama.cpp servers to be down (starting, running and stopping all count), the audio.cpp
updater for the TTS server; each looks only at its own. The button is disabled and a red
line says which server and what to do. The panel updater needs no such rule.

v3.76 patch35: the Audio Cache popup grows to fit its content instead of
clipping the long rows. Clicking a character's voice sample filename now
plays that sample; clicking anywhere else on the row still opens their kept
takes, and take rows still play on click. When a playback is refused, the
row says why (hover it) instead of staying silent. And voice pre-warming on
a speaker switch now actually runs - a locking mistake meant it silently
never did, which is why the 'tag strip' fallback was the only thing you
ever saw in the log.

v3.76 patch34: thought audio no longer overlaps anyone. A thought now plays
right after its own reply's last spoken chunk, and the next character's
reply is held back until the thought has actually finished sounding in your
browser - so the old collision, where a new line talked over a lingering
thought, is gone by construction. The Sequence choice (before/after) is
retired; in its place is a Delay field: the seconds between a chunk being
handed over and you hearing it. It ships at 0.75. If a thought ever clips
the tail of its own line, raise it a touch - being late costs nothing now.

v3.76 patch33: two fixes for typed and switched voices. First, Voice Prime's
tag-strip fallback in patch31 ran before the panel had translated your typed
tags into engine tokens, so on an unprimed voice switch it removed nothing -
the cut now happens after translation, where the tokens actually are. Second,
every emotion the Higgs engine accepts can now be typed in a line: alongside
the existing words, [affectionate] [amused] [aroused] [awed] [bitter]
[contemplative] [contented] [determined] [helpless] [longing] [relieved] and
[ashamed] (noun forms work too) all convert; [content] alone stays ordinary
dialogue, being too common a word.

v3.76 patch32: the Audio Cache keeps house. Kept audio files are now capped
per character - fifty by default, the oldest replaced as a new line arrives,
so long play sessions no longer grow your output folder without limit; the
cap is on the TTS page (blank or 0 keeps everything). In the Audio Cache
popup, every kept take is now playable - click a row to listen to it right
there, with its creation date beside it as before.

v3.76 patch31: Voice Prime. Higgs can misrender a speaker's identity on the
first chunk after a voice change when audio tags lead the line - a male NPC
briefly sounding female. With Voice Prime on (TTS page, on by default), the
panel warms the incoming voice with one hidden, discarded micro-utterance
while the dialogue reply is still being written, so the first thing you hear
is already stable - at no added latency. If a switch arrives before the
warm-up could run, that line's first-chunk audio tags are dropped instead of
making you wait; the TTS terminal notes both events. Off restores the old
behaviour exactly.

v3.76 patch30: developer documentation only - nothing changes on screen or
in behaviour. The design record of the presentation work from the last six
patches is written into the developer notes.

v3.76 patch29: a counted reasoning figure no longer wears the ~ estimation
mark on the Proxy record line. When llama.cpp reports how many tokens the
reasoning cost, the line now shows (412); the ~ appears only when the panel
had to estimate from the text, which is what it always meant. The Thinking
header has written it this way since v3.76 patch13 - the record line and the
header now agree.

v3.76 patch28: internal groundwork only - nothing on screen changes. A
provider's colour is now worked out once, when the routing is read, and
every place that shows a provider - the terminals, the network cards, the
charts, the chips - asks that one answer instead of re-deriving it on every
glance. Every provider keeps exactly the colour it had.

v3.76 patch27: internal groundwork only - nothing on screen changes. Spoken
lines, thought lines and the TTS report rows now take every colouring
decision from the same declared tables as the other terminals, wherever the
line appears - the Proxy terminal and the TTS terminal read one declaration
for the same element, which was always the intent and is now the mechanism.
Anything undecided shows up marked and reported instead of wearing a quiet
default. Every terminal draws exactly what it drew before.

v3.76 patch26: internal groundwork only - nothing on screen changes. The
Thinking, PTI/PME and TTS Calibration terminals now take every colouring
decision from the same declared tables the Proxy terminal moved to in the
previous patch, and anything left undecided shows up marked in the terminal
and reported in the panel log instead of quietly wearing a default. Every
terminal draws exactly what it drew before.

v3.76 patch25: internal groundwork only - nothing on screen changes. How each
terminal recognises what it shows and which colour it gives each thing is now
written down in one place inside the panel, checked for gaps every time the
page loads, and any gap is reported in the panel log instead of drawing
silently wrong. The Proxy terminal is the first to run on it; the others
follow in coming patches. Every terminal draws exactly what it drew before.

v3.76 patch24: internal housekeeping only - nothing on screen changes. The
checks that guard how the terminals draw were strengthened, and the developer
notes were brought back in line with the files that actually ship.

v3.76 patch23: the realtime reading on a spoken line shown in the Proxy terminal
is coloured like it is everywhere else. With Dialogue Text switched on, the 4.42x
at the end of a spliced line was plain grey while the same figure was gold in the
TTS terminal.

v3.76 patch22: turning timestamps on in the TTS terminal no longer breaks spoken
lines. With them shown, the speaker's name lost its colour and the mood icon lost
the fixed cell that keeps names lined up under each other. Both are back, and the
timestamp itself now matches the colour every other terminal gives it.

v3.76 patch21: timestamps look the same in every terminal - the Proxy terminal
used a slightly different blue from the other four. Spoken lines and thoughts now
take their colours from the same place every terminal reads, so a change lands
everywhere at once. Nothing else moved: each terminal keeps its own colouring for
numbers, durations and anything it alone shows.

v3.76 patch20: a thought no longer holds the next line back longer than it needs
to. The panel was estimating how long a thought would take to play, from the length
of the file, and waiting out that estimate. The page now reports the moment it
actually finishes, so the wait ends when the thought does. If a page is closed or
asleep and never reports, the old estimate still applies, exactly as before.

v3.76 patch19: internal tidying, with no visible change. The TTS terminal kept
its own copy of three colours the rest of the panel already shared, and now reads
the shared set, so a colour changes in one place or not at all. Every line the
terminal drew before it draws identically after.

v3.76 patch18: timings no longer count time the machine spent asleep. Durations
the panel measures - how long a request took, how long a model generated, how long
speech took to produce - were read from the system clock, so a sleep or a clock
adjustment in the middle of one was counted as part of it, and one log showed a
status read taking six and three quarter hours. They now use a clock that only
moves forward.

v3.76 patch17: clicking a provider in the Proxy terminal opens its request and
reply again - it had started jumping to Live Network instead once the provider list
finished loading. Headings in the Thinking and PTI/PME terminals now show the
provider as the link rather than a tag reading "Request", which they only did when
timestamps were shown - off by default in those terminals - and that tag errored
when clicked. The player's voice and the player's thoughts can also share one
recording now; they are the same person, but the panel had been treating the second
one to arrive as an impostor and refusing the line.

v3.76 patch16: a provider looks the same in every terminal. Its name only ever
carried its colour and emoji in the Proxy terminal - in the Thinking, PTI/PME and
Calibration feeds it was plain grey text. Every terminal now draws it from the same
list, with the same colour, the same emoji and the same hover glow wherever the
name appears, and clicking it opens that provider in Live Network. A provider whose
name is part of a longer word is left alone, so one called Vision does not light up
inside "Visionary".

v3.76 patch15: a spoken line links to the call its words came from. Every line
in the TTS terminal carries a Request tag, and clicking it opens the request and
reply that produced those words. A line too short to belong to one reply, or whose
words appear in two, carries no tag rather than a wrong one. Replay is untouched,
and the copy of a spoken line shown under its completion in the Proxy terminal
carries no second link - that completion's own line already opens the request.

v3.76 patch14: completed reasoning shows which provider produced it, as a link.
Each heading in the Thinking terminal carried a tag reading "Dialogue" beside the
provider's name, whatever that provider actually was. The provider's own name is
now the link, and clicking it opens that call's request and reply - the same as
clicking the provider on the call's line in the Proxy terminal. The PTI/PME
terminal works the same way, on the name of the job. An action branch that carried
a second link to the request its own record line already opens no longer does.

v3.76 patch13: the reasoning token count agrees with itself. The figure on a
call's record, on its heading in the Thinking terminal and in its payload window
came from three separate calculations, and two could differ from the third by one.
They also estimated from the length of the reasoning even on calls where llama.cpp
had counted the tokens exactly. A figure that was counted now reads (412 tok); one
the panel measured still reads (~398 tok est).

v3.76 patch12: the bad-match warning on draft models and vision projectors
works again. The panel remembers what it read out of each model's header, and two
releases ago it started reading one more thing - the width a file speaks at -
without telling that memory, so every model listed before then had nothing for the
check to compare and it stayed quiet. Those entries are re-read on the next start,
which makes that one start slower, and a mismatched drafter or projector is marked
again.

v3.76 patch11: the Thinking link in the Proxy terminal works. A call that
produced reasoning was printing its link marker as raw text instead of showing a
Thinking tag - clicking it now scrolls the Thinking terminal to that call's
reasoning. The action a character chose carries a Dialogue tag of its own, so the
branch opens the request and reply it came from, and a link followed in split view
lights the pane you are looking at rather than one behind it.

v3.76 patch10: the mismatch warning for draft models and vision projectors is
back and now catches whole models carrying an MTP head, which the previous version
passed over in silence. The check asks whether a file attaches to the model rather
than which metadata field it happened to fill in, so a plain draft model is still
left alone.

v3.76 patch9: the player's inner monologue is spoken again. SkyrimNet sends
player thoughts on their own voicetype, which the panel did not recognise as the
player's - so those lines were refused as belonging to somebody else and no audio
came back. A server that runs out of memory for its KV cache now says so
specifically rather than blaming the model.

v3.76 patch8: the player tags and mood terminal joins the cross-terminal
links. A PTI or PME record carries a Dialogue tag pointing at the request and
reply it came from, so a bad tag can be traced straight back to what the model was
asked.

v3.76 patch7: the jump between terminals goes both ways. A call in the Proxy
terminal that produced reasoning carries a Thinking tag - clicking it scrolls the
Thinking terminal to that call and flashes it. The Dialogue tag opens the request
and reply from the other side. A block that has scrolled out of the window says so
rather than doing nothing.

v3.76 patch6: a record in one terminal can point at the same record in
another. The Thinking terminal's block for a call now carries a Dialogue tag, and
clicking it opens that request and reply. The reference is written where the panel
knows what the thing is rather than guessed at from the text, and the same
identifier appears on both sides. Speech, player tags and mood records follow.

v3.76 patch5: Terminate now reaches a speech server this panel started in an
earlier session. The panel recognised its own server only while it stayed open, so
restarting it left the server running and out of reach. It is taken back on
startup, and only when the process, the executable and the port all still agree -
a server the panel never launched is still left alone.

v3.76 patch4: a server that fails to start now says why - out of video
memory, a draft model or projector that does not fit, an unknown architecture, a
port already in use, and so on. The card used to quote the last line of the log,
which is what the engine was doing when it stopped rather than why, and the real
reason was usually a line above it.

v3.76 patch3: vision projectors are now checked against the width they
actually project to - the field llama.cpp compares - so a projector built for a
different size model is marked before you pick it, across Gemma 4 and Qwen 3.6 and
3.8. A plain draft model is no longer marked wrong simply for being a different
size, which was never a fault. The budget message box matches the settings around
it and grows while you type in it.

v3.76 patch2: an error in the page now records which functions were running,
not just a line number, and says so on screen instead of only in a file. A single
server card that fails to draw no longer blanks the whole page - it is drawn as a
card explaining itself while the rest of the fleet appears as normal.

v3.76 patch1: the server cards are back - a line added in hotfix2 referred to
a name that did not exist where it was written, and every card threw while being
drawn. The build now renders the page before it passes, so a fault that only
appears when a line runs cannot ship again. Numbers, durations, ports, timestamps
and character names are also described in one place rather than separately inside
each terminal.

v3.76 hotfix4: your character's name is now read from the first line of
dialogue instead of being worked out over several. The panel had been looking for
a party heading that SkyrimNet only writes when you have a party; it now reads the
line that names you as the player character, with the inventory and combat
assessment behind it. Character names also colour correctly in the Thinking
terminal - the list of known characters was reaching the terminals only while
viewing another machine's panel.

v3.76 hotfix3: the reasoning budget message can be set on the server card.
llama.cpp injects this line when a model spends its whole thinking budget; the
Sampler Guide already described it but there was nowhere to change it. Type your
own beside Reasoning budget and it goes into the launcher. Left empty, llama.cpp
keeps its own wording.

v3.76 hotfix2: a draft model or vision projector built for a different model
is now marked yellow in the dropdown before you pick it, with a pulsing "Bad LLM
Match" label if you pick it anyway - these mismatches used to reach llama.cpp,
which accepts them and then stops with a raw assertion naming neither file. A
server that fails to start also gives its Launch button back instead of leaving
the card stuck on "Launching Server..." until the page was reloaded.

v3.76 hotfix1: the panel works out your character's name far more reliably -
it no longer accepts placeholders like "someone", and better evidence now
replaces an earlier guess instead of being locked out. A line spoken in the
player's own voice is matched against the replies the panel carried, which names
the player from its own record. Token counts return to the payload window,
reasoning included. Every terminal now paints numbers and character names from
one shared table.

v3.76 Beta: every speculative decoding setting on a server card now opens
its own page in the Sampler Guide - those seven flags were drawn as links and
did nothing. Four new pages cover the drafter's minimum tokens, split
probability, backend sampling and its separate KV cache precision. The guide
also gains every repetition penalty llama.cpp offers - repeat penalty, its
look-back window, presence penalty and frequency penalty - alongside the DRY and
XTC pages it already had.

v3.75 patch44: the repository states its own line endings. The exact bytes
of every file are part of what the project checks, but nothing told git that, so
a copy taken with git clone was formatted by whatever setting the copying machine
happened to use - which on a common Windows setup gives the wrong endings for the
documentation and one launcher script. Only affects working from a clone; an
installation unpacked from the release zip was never involved.

v3.75 patch43: the six emotion tags that ship switched off now reach an
existing installation as well as a fresh one. A tag board that had ever been
adjusted kept its old contents on upgrade, so the six stayed on for exactly the
people using tags most. They are added to what is already there, once, and
anything switched back on afterwards stays that way.

v3.75 patch42: six emotion tags - disgust, elation, longing, sadness, shame
and determination - ship switched OFF rather than removed. They stay on the
Allowed Tags board and one click brings any of them back. They are not offered to
the tag injector or the mood reader, and switching an emotion off now stops NPC
lines as well as the player's, which it never did before.

v3.75 patch41: five emotion tags - disgust, longing, sadness, shame and
elation - are no longer used. Measured one line at a time, the first four make
Higgs speak as a different person and the fifth loses the speaker entirely. They
are refused on the wire and removed from the tag injector and mood reader
prompts. A character speaking through another character's unique voice sample is
now called by their own name when the words of the line say so.

v3.75 patch40: a character's line is never announced under the player's name -
the guard that refuses a name belonging to another voice sample could not see
the player's own, which is the name SkyrimNet queues most often. A queued line
that could not have been this character's no longer leaves the true speaker
unnamed. Any take that runs longer than the sample it was cloned from is now
recorded, in the terminal and the log.

v3.75 patch39: an NPC's thought now waits for the last chunk of the line it
belongs to, even when that chunk is slow to synthesize - a chunk that has been
asked for and not yet answered counts as the reply still arriving. A thought is
synthesized once rather than twice, and the next character's line waits for a
thought that is still being spoken. A character whose name runs past 29
characters - "Ertzebet the Librarian's Assistant" - is read correctly; their
lines had been filed with no speaker at all, which silently cost them their
emotion tags, their thought audio and their name in the identity ledger. The
Providers and Options panels take the Adjust panel's glow and geometry, and
All on/off is centred.

v3.75 patch38: when an NPC's thought lands earlier than it should, the TTS
calibration log now says why - whether the reply it compares against was
missing, stale, or did not match. Actions are shown by default, and action
parameters are no longer cut short.

v3.75 patch37: SkyrimNet's startup ping is Banned by default, NPC thoughts
are shown by default, terminals start without the time column, and a
terminal's Providers panel gains a button that turns them all on or off -
with the providers wrapped into an even grid.

v3.75 patch36: player lines are tagged again - PTI and PME have no port of
their own, and the readiness check was asking about it instead of about their
server. Remote Access now takes effect the moment you switch it rather than
at the next restart, and installing Higgs fills the folder and model fields
without reloading the page.

v3.75 patch35: every server card setting is editable - the card still says
when one setting overrules another, but no longer takes the control away.
Installing or adopting Higgs reloads the page, so the folder paths and the
model selection appear without pressing CTRL+F5.

v3.75 patch34: context checkpoints now default to 0. They are rollback copies
of a slot's context kept in system memory - llama.cpp keeps eight per slot -
and they were the last reason a model borrowed RAM after the GPU pin and
load mode were fixed. A default server launcher now keeps everything on its
own card and fails at launch if it does not fit.

v3.75 patch33: Model load mode now defaults to dio, which is what keeps each
model on its own pinned card - memory-mapped weights stay host-resident and
llama.cpp spreads them across every card it can reach. The setting is always
editable again, and installing or adopting Higgs repaints the TTS page at
once instead of waiting for a restart.

v3.75 patch32: a server with no GPU chosen is assigned one when its launcher
is written - least-loaded card first, logged, and shown on the card - so a
fresh install pins correctly without any setup. The installed Higgs model is
selected in its dropdown again, clicking a detected IP fills the field, and
the TTS page repaints after an install even while a button holds focus.

v3.75 patch31: the GPU pin is now written into the launcher file itself
rather than left to a template, so a server runs on the card its own card
names and no other. Installing or adopting Higgs also fills every folder
path it found - engine, models and Sample Vault - and the setup-page
addresses respond to a click.

v3.75 patch30: a server set to "Auto fit to VRAM: off" now tells llama.cpp
so - it used to write nothing, and llama.cpp fits by default, which is why
models spread across every card and into system RAM. The fleet also has its
own launcher templates in server-templates\, all of which pin one card and
keep the model on it; the Launcher Creator's templates stay in templates\
and are no longer offered to server cards.

v3.75 patch29: the voice warm-up is gone - it cost about three quarters of a
second on every speaker change and the carryover it fought went away with the
reference samplers. Proxy Setup addresses are readable, Saved Audio Folder
shows a full path, reverting TTS settings no longer forgets where the engine
is installed, recognition defaults to Auto, and server-card launchers report
how they ended.

v3.75 patch28: the TTS page updates itself after an install, Dismiss works,
and a Revert to default TTS settings button sits beside the TTS selector.
Saved audio defaults to a TTSaudio folder beside the panel, the 10-second
sample window survives installs, long lines split at 140 characters, the
setup-page addresses are shown plainly, and the Permission Tree matches this
build.

v3.75 patch27: server cards now carry Model load mode instead of Disable
mmap. Current llama.cpp folded --no-mmap, --mlock and --direct-io into one
--load-mode flag and warns on the old spellings; the setting offers exactly
the values it accepts and defaults to auto. The Sampler Guide has a page for
it, and every card cross-reference was repointed.

v3.75 patch26: the default launcher stops relaunching itself when a server
fails, keeps its window open so the error stays readable, carries the exit
code, and prints the line the panel needs to tell a dead server from a
loading one. llama.cpp's own output still reaches the log unchanged, which
is what the VRAM report is read from.

v3.75 patch25: a server that fails to start now explains itself - the card
reads the slot's own log and shows what the server last wrote before it went
away, instead of sitting silently on "down". DFlash drafters are also asked
for the draft depth they can actually produce, read from the drafter's own
header and tensors, so the engine no longer reduces it on every launch.

v3.75 patch24: SenseVoice transcription is timed and shown by name on the
timing line, the graphic bar and the calibration terminal. The bar itself now
draws whatever was spent rather than a fixed set of bands, so thought
synthesis, the sample gate and the rest finally appear on it.

v3.75 patch23: TTS Samplers is temperature and top-k, set by hand - Boson's
reference pair for this model, with top-p, min-p and repetition penalty
removed because Higgs either ignores them or is harmed by them. Sampler
Calibration and Steady Retry are gone: both moved these values, and moving
them is what produced the endless repeated-token takes. The proxy still sets
the per-line token limit, and a retry still raises it.

v3.75 patch22: the player tagger is told that most lines need no tags and
answers with silence unless the words clearly call for one - a flat sentence
no longer comes back marked as singing. It also skips instantly when its
server is not running instead of stalling every player line on a timeout,
and says once which server it is waiting for.

v3.75 patch21: the speech samplers now default to Boson's own reference
values for Higgs - temperature 0.8, top-k 50, nothing else sent - and Top-K
is a setting of its own. The retry no longer steps the samplers colder each
attempt: that walked them toward greedy decoding, which is what produces the
endless repeated-token takes in the first place. Installing Higgs restores
these values over whatever the fields held.

v3.75 patch20: the token-cap margin can finally move. It was measuring
that it needed about 2.65 and being clamped to 1.60 every time, which is
why the retry ladder kept rescuing lines that never got a bigger cap next
time. There is also a Clear calibration data button on the TTS Calibration
section, with a confirmation explaining what it forgets and what it leaves
alone.

v3.75 patch19: only one line reaches the speech engine at a time. Its
reference state lives in the model session, so a dialogue line and a thought
arriving together could cross - one voicetype speaking in another's voice.
Waiting for the engine is now shown on its own line instead of counting as
synthesis. An allocation refusal also stops raising the token cap, which was
asking the card for a bigger buffer than the one it had just refused.

v3.75 patch18: the timing report now measures the synthesis that actually
played instead of the whole retry ladder, so a line that failed once and
succeeded once no longer reads as one slow take - discarded attempts get
their own line. Every line also writes a row to tts-timing.csv with tokens,
timings, attempts and the reference sample's length, so the per-request and
per-token costs can be told apart from real play.

v3.75 patch17: diagnostics. audio.cpp is started with --log and the panel
writes its own launch banner - every model, family, size, the pinned card,
what is resident on it, and the argv - so a failed session can be read
afterwards. Failures carry stable codes (TTS-E01..E06), an allocation
failure is retried instead of silently losing the line, every audio-path
step reports its own milliseconds by name, and the SenseVoice picker no
longer offers speech models.

v3.75 patch16: a take suspected of dropping its words is transcribed and
judged on them - convicted takes retry once, acquitted ones are kept.
Vocal performances ("Ahh...", sighs, murmurs) pass unjudged in both
directions and no longer teach the pricing model. Sound-effect and
ellipsis costs are learned from real lines, the runaway verdict runs on
the same per-voice model as the token cap, and the Meta tags setting -
which changed nothing visible - is removed.

v3.75 patch15: the per-line token cap is now fitted per voicetype instead
of guessed from one constant. Each voice learns its own overhead from the
takes it has produced, sharing a per-character rate with every other voice.
Short lines get more room - they were the ones being turned into failures -
and long lines get less, so a runaway is cut off sooner. Nothing to set up:
a new voice uses the shipped fit and adapts as it speaks.

v3.75 patch14: reference transcripts are on by default and the Higgs
installer downloads SenseVoice for you (about 254 MB) and points the field
at it. Every control in the section now has a ? explanation, picking the
wrong gguf as the recogniser is flagged in red, and the TTS page refreshes
itself the moment an install completes.

v3.75 patch13: Runaway Handling is a dropdown in TTS Backend Settings and
detection is the default - every take's audio is measured against what its
text should run; a runaway is retried once and the better take speaks, a
suspiciously short take is logged. Line Time Limit remains as the alternate
mode. The Sample Vault is now the default voice home: the installer creates
it and sets the path, Local Voice Clips act as an alternate source that is
repaired into the vault and re-adopted whenever you edit a clip, and Cached
Voices defaults to 1024.

v3.75 patch12: the server cards are visually consistent - equal spacing at
every group boundary with the divider centred in it, and the Speculative
Decoding section styled exactly like the groups above it, blue flag
references included. A model with its own MTP head shows a yellow
"Built-in MTP head" chip under the model picker and beside the Speculative
Decoding heading when that head is the drafter.

v3.75 patch11: the panel now keeps its own repaired copy of every voice it
meets. The first time a character speaks, their clip is read - even if its
header declares an impossible length, which is what made a lot of voice
files unplayable - rewritten correctly, trimmed to a short example, and
saved under the voicetype's name in a vault folder. Every later line speaks
from that copy. Your original files are never modified. Two rows in Audio
Files choose where the vault lives and how long a kept sample may run.
Hovering a provider title in the proxy terminal also glows properly now
instead of blurring.

v3.75 patch10: Reference Transcripts is now its own settings field on the
TTS page, above Audio Tags - Off, Stored only (use what is known, load no
model), or On (co-host SenseVoice and learn the rest), with language, meta
tag and number-format controls and a Clear transcripts button. Nothing ever
waits on it: the speak path reads what is known and unknown samples are
transcribed in the background, so the first line for a new voice goes out
at once and the next one carries its transcript. The store keeps the
voicetype beside each line so you can correct it by hand, which beats any
recogniser on Skyrim names.

v3.75 patch9: server cards grow a Speculative Decoding segment whenever a
drafter is selected - draft tokens per step, acceptance thresholds, backend
draft sampling, and (for file drafters) the drafter's own KV cache types,
all reading back from hand-edited launchers. Qwen 3.8 is fully supported:
its multi-step MTP head is auto-detected for the built-in drafter, and the
card shows both the Reasoning on/off dial and a Reasoning effort select
(low / medium / xhigh) - Off outranks effort.

v3.75 patch8: ready for audio.cpp 0.6 - the start retries once without
session options when the server refuses them, so upgrading the engine costs
seconds, not a working Start button. An optional SenseVoice model
(TTS page, "ASR model for reference transcripts") is co-hosted beside the
TTS engine and transcribes each voice sample once; every cloning request
then carries the transcript, which locks voice identity harder. And the
Audio Files settings grew two buttons: Audio Cache lists every character
the panel has ever met with the sample that speaks them - the pairings
persist across restarts - with click-through to every kept take, dated,
newest first; Clear Audio Cache forgets the learned list, disk copy
included, behind a confirm.

v3.75 patch7: the panel now mirrors what the TTS engine's single session
was last conditioned on, across every synthesis path - voiced thoughts
included - and each block in tts-identity.log says "engine was: <voice>"
whenever the previous take was somebody else's. One read of that line tells
a wrong-voice case apart: nothing there means a cold start (the engine's own
misrender), another voicetype there means carryover from the previous
speaker. The voice warm-up switch reads the same record, so a thought voiced
between two of your lines counts as the speaker change it is.

v3.75 patch6: every voice sample is verified before the TTS engine sees
it. A reference missing from disk is recovered from the panel's own copy of
that voicetype or the line is refused loudly, and a path carrying another
voicetype's bytes - proven by hash - is treated the same way; a re-recorded
sample is accepted with one alarm. The gate's verdict lands in
tts-identity.log beside the fingerprint. There is also a voice warm-up
switch on the TTS page, off by default: a short discarded take on each NEW
speaker's voice before their real line, absorbing the engine's carryover
from the previous one - turn it on and, if first chunks in the wrong voice
stop, the carryover theory is proven on your own hardware.

v3.75 patch5: a spoken line is named by its own words first - every reply
the proxy serves is remembered, four deep per character, and a TTS chunk is
matched into exactly one of them, however late the TTS queue delivers it.
Chunks of one utterance travel together, so a short "Not an inch." inherits
the name its longer sibling proved. Only Dialogue requests feed the pairing
queue now; GM, Combat and Meta traffic no longer lend their names to passing
lines. A unique voicetype binds its character permanently from its own
filename, the Meta selector's speaker>listener pick breaks ties among
waiting requests, and a generic voicetype with nothing live behind it prints
as itself instead of wearing the previous speaker's name. The player can
also be recognised automatically as the one name everyone addresses and
nobody ever is. Every distinct TTS request shape SkyrimNet sends is recorded
in tts-identity.log - keys and header names only - so what crosses the wire
is a matter of record.

v3.75 patch4: your character's name is a setting - a field under the Player
Tag Injector labels your own spoken lines and tells the tagger who "I" is,
and typed there it outranks anything read from a prompt. The rule that could
learn the player's name from a think request is gone; the party heading is
the only prompt that names them, the source is recorded, and a name that
already belongs to a character is refused. A spoken line under 24 characters
names nobody, and a name bound to one voice sample can never be moved onto a
second. Every spoken line now writes a block to tts-identity.log naming the
speaker, the rule that decided it, the sample sent with its size and hash,
and the line itself; a reference missing from disk is called missing, and
one character arriving on a second sample is alarmed in the TTS terminal as
it happens.

v3.75 patch3: a model that carries its own MTP head can now be its own
drafter. The Speculative decoding picker offers "Built-in MTP head", and the
launcher gets "--spec-type draft-mtp" with no draft file at all; a launcher
already written that way shows up on the card as that choice. A dialogue
reply that opens with its internal thought is served dialogue-first, the
thought placed right behind the first spoken segment, so the words reach TTS;
thought-only replies keep their shape, and thoughts are shown and voiced in
full. Every server card now carries a per-launch VRAM report above its
terminal - weights, drafter, projector, KV, RS, compute and MTP context, plus
GPU free at load - read from llama.cpp's own log lines, and a launch that
died before serving says so in red instead of reporting itself loaded. The
port warning tells a stuck or dying load apart from a foreign process
holding the port.

v3.75 patch2: a speculative drafter is typed by what its tensors prove
rather than by the architecture it declares, matching how llama.cpp
itself identifies one - so qwen35-generation MTP heads launch as
draft-mtp and Markov-headed DFlash files as draft-dspark. A model that
carries its own MTP head is labelled on its card: it can draft for
itself with no separate file. The launcher check also flags an integer
flag handed a fraction, such as a top-k of 0.95.

v3.75 patch1: releases are now ordered by kind as well as number - a
release, then its hotfixes, then its patches - so the update check can
tell a hotfix and a patch of the same number apart.

v3.75 hotfix1: a selected speculative drafter is now launched with
its decoding type, without which recent llama.cpp builds load the drafter
and draft nothing. Gemma 4, DFlash and EAGLE-3 heads and plain small
drafters each get the type they need.
